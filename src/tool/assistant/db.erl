...
save(Head, Format, Tail, Data) ->
    save_into(Head, Format, Tail, Data, fun(A) -> A end, 0).
...
execute(Sql) ->
    Worker = volley:get(?MODULE),
    ?QUERY(mysql_connector:execute(Sql, Worker, ?MINUTE_MILLISECONDS)).
...
-spec collect(Sql :: mysql_connector:sql(), Value :: mysql_connector:sql(), Update :: mysql_connector:sql(), Binding :: list() | ets:tab(), Collect :: fun((term()) -> list()), Filter :: non_neg_integer()) -> binary().
%% list
collect(Head, Format, Tail, Binding, Collect, Flag) when is_list(Binding) ->
    collect_list_loop(Head, Format, Tail, Binding, Collect, Flag, <<>>);
%% ets
collect(Head, Format, Tail, Tab, Collect, Flag) when is_atom(Tab) orelse is_reference(Tab) ->
    ets:safe_fixtable(Tab, true),
    Key = ets:first(Tab),
    Object = ets:lookup(Tab, Key),
    collect_ets_loop(Head, Format, Tail, Tab, Key, Object, Collect, Flag, <<>>).
...
collect_list_loop(_, _, _, [], _, _, <<>>) ->
    <<>>;
%% end of list
collect_list_loop(Head, _, Tail, [], _, _, Acc) ->
    <<Head/binary, Acc/binary, Tail/binary>>;
%% element
collect_list_loop(Head, Format, Tail, [H | T], Collect, 0, <<>>) ->
    Data = format(Format, apply(Collect, [H])),
    %% change update/save flag
    collect_list_loop(Head, Format, Tail, T, Collect, 0, Data);
%% first element
collect_list_loop(Head, Format, Tail, [H | T], Collect, Flag, <<>>) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    collect_list_loop(Head, Format, Tail, T, Collect, Flag, Data);
%% normal element
collect_list_loop(Head, Format, Tail, [H | T], Collect, Flag, Acc) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% insert delimiter
    collect_list_loop(Head, Format, Tail, T, Collect, Flag, <<Acc/binary, ",", Data/binary>>);
%% other element
collect_list_loop(Head, Format, Tail, [_ | T], Collect, Flag, Binary) ->
    collect_list_loop(Head, Format, Tail, T, Collect, Flag, Binary).

%% ets
%% without value do not concat head and tail
collect_ets_loop(_, _, _, Tab, '$end_of_table', [], _, _, <<>>) ->
    ets:safe_fixtable(Tab, false),
    [];
%% end of table
collect_ets_loop(Head, _, Tail, Tab, '$end_of_table', [], _, _, Acc) ->
    ets:safe_fixtable(Tab, false),
    %% end of table
    <<Head/binary, Acc/binary, Tail/binary>>;
%% element
collect_ets_loop(Head, Format, Tail, Tab, Key, [H], Collect, 0, <<>>) ->
    Data = format(Format, apply(Collect, [H])),
    %% next
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    collect_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, 0, Data);
%% first element
collect_ets_loop(Head, Format, Tail, Tab, Key, [H], Collect, Flag, <<>>) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% next
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    collect_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, Flag, Data);
%% normal element
collect_ets_loop(Head, Format, Tail, Tab, Key, [H], Collect, Flag, Acc) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% next
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    %% insert delimiter
    collect_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, Flag, <<Acc/binary, ",", Data/binary>>);
%% other element
collect_ets_loop(Head, Format, Tail, Tab, Key, _, Collect, Flag, Acc) ->
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    collect_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, Flag, Acc).

%% @doc save into
-spec save_into(Sql :: mysql_connector:sql(), Value :: mysql_connector:sql(), Update :: mysql_connector:sql(), Binding :: list() | ets:tab(), Collect :: fun((term()) -> list()), Filter :: non_neg_integer()) -> list() | ok.
%% list
save_into(Head, Format, Tail, Binding, Collect, Flag) when is_list(Binding) ->
    save_into_list_loop(Head, Format, Tail, Binding, Collect, Flag, <<>>, []);
%% ets
save_into(Head, Format, Tail, Tab, Collect, Flag) when is_atom(Tab) orelse is_reference(Tab) ->
    ets:safe_fixtable(Tab, true),
    Key = ets:first(Tab),
    Object = ets:lookup(Tab, Key),
    save_into_ets_loop(Head, Format, Tail, Tab, Key, Object, Collect, Flag, <<>>).

%% list
%% without value do not concat head and tail
save_into_list_loop(_, _, _, [], _, _, <<>>, List) ->
    List;
%% end of list
save_into_list_loop(Head, _, Tail, [], _, _, Acc, List) ->
    query(<<Head/binary, Acc/binary, Tail/binary>>),
    List;
%% element
save_into_list_loop(Head, Format, Tail, [H | T], Collect, 0, <<>>, List) ->
    Data = format(Format, apply(Collect, [H])),
    %% change update/save flag
    save_into_list_loop(Head, Format, Tail, T, Collect, 0, Data, [H | List]);
%% first element
save_into_list_loop(Head, Format, Tail, [H | T], Collect, Flag, <<>>, List) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% change update/save flag
    New = erlang:setelement(Flag, H, 0),
    save_into_list_loop(Head, Format, Tail, T, Collect, Flag, Data, [New | List]);
%% normal element
save_into_list_loop(Head, Format, Tail, [H | T], Collect, Flag, Acc, List) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% change update/save flag
    New = erlang:setelement(Flag, H, 0),
    %% insert delimiter
    save_into_list_loop(Head, Format, Tail, T, Collect, Flag, <<Acc/binary, ",", Data/binary>>, [New | List]);
%% other element
save_into_list_loop(Head, Format, Tail, [H | T], Collect, Flag, Binary, List) ->
    save_into_list_loop(Head, Format, Tail, T, Collect, Flag, Binary, [H | List]).

%% ets
%% without value do not concat head and tail
save_into_ets_loop(_, _, _, Tab, '$end_of_table', [], _, _, <<>>) ->
    ets:safe_fixtable(Tab, false),
    [];
%% end of table
save_into_ets_loop(Head, _, Tail, Tab, '$end_of_table', [], _, _, Acc) ->
    ets:safe_fixtable(Tab, false),
    %% end of table
    query(<<Head/binary, Acc/binary, Tail/binary>>),
    [];
%% element
save_into_ets_loop(Head, Format, Tail, Tab, Key, [H], Collect, 0, <<>>) ->
    Data = format(Format, apply(Collect, [H])),
    %% next
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    save_into_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, 0, Data);
%% first element
save_into_ets_loop(Head, Format, Tail, Tab, Key, [H], Collect, Flag, <<>>) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% change update/save flag
    ...
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    save_into_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, Flag, Data);
%% normal element
save_into_ets_loop(Head, Format, Tail, Tab, Key, [H], Collect, Flag, Acc) when element(Flag, H) =/= 0 ->
    Data = format(Format, apply(Collect, [H])),
    %% change update/save flag
    ets:update_element(Tab, Key, {Flag, 0}),
    %% next
    ...
    Object = ets:lookup(Tab, Next),
    %% insert delimiter
    save_into_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, Flag, <<Acc/binary, ",", Data/binary>>);
%% other element
save_into_ets_loop(Head, Format, Tail, Tab, Key, _, Collect, Flag, Acc) ->
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    save_into_ets_loop(Head, Format, Tail, Tab, Next, Object, Collect, Flag, Acc).
...
format(Sql, Binding) ->
    format(iolist_to_binary(Sql), Binding, <<>>).

%% list arguments
format(<<>>, [], Acc) ->
    Acc;
format(<<$?, Binary/binary>>, [Arg | Args], Acc) when is_integer(Arg) ->
    Data = integer_to_binary(Arg),
    format(Binary, Args, <<Acc/binary, Data/binary>>);
format(<<$?, Binary/binary>>, [Arg | Args], Acc) when is_float(Arg) ->
    Data = list_to_binary(io_lib_format:fwrite_g(Arg)),
    format(Binary, Args, <<Acc/binary, Data/binary>>);
format(<<$?, Binary/binary>>, [Arg | Args], Acc) when is_function(Arg) ->
    Data = apply(Arg, []),
    format(Binary, Args, <<Acc/binary, Data/binary>>);
format(<<$?, Binary/binary>>, [Arg | Args], Acc) ->
...
serialize(Value) when is_list(Value) ->
    case catch unicode:characters_to_binary(Value, utf8) of
...
get_auto_increment(Table) ->
    select_one(format(<<"SELECT AUTO_INCREMENT FROM information_schema.`TABLES` WHERE `TABLE_SCHEMA` = DATABASE() AND `TABLE_NAME` = ?">>, [Table])).
