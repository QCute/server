...
-export([save_into/3, save_into/4, save_into/5]).
...
%% @doc save
-spec save(Sql :: mysql_connector:sql(), Value :: mysql_connector:sql(), Update :: mysql_connector:sql(), Binding :: list() | ets:tab(), Collect :: fun((term()) -> list()), Filter :: non_neg_integer()) -> list() | ok.
save(Sql, Value, Update, Binding, Collect, Filter) ->
    save_into_loop(Sql, Value, Update, Binding, Collect, Filter).
...
-spec collect(Data :: list() | ets:tab(), Head :: binary(), Format :: binary(), Tail :: binary()) -> Sql :: binary().
collect(Data, Head, Format, Tail) ->
    %% tuple sql
    collect_loop(Data, Head, Format, Tail).

%% @doc collect transform data
-spec collect(Data :: list() | ets:tab(), Head :: binary(), Format :: binary()) -> Sql :: binary().
collect(Data, Head, Format) ->
    %% pair sql
    collect_loop(Data, Head, Format, <<>>).

%% @doc collect transform data
-spec collect(Data :: list() | ets:tab(), Head :: binary()) -> Sql :: binary().
collect(Data, Sql) ->
    %% single sql
    collect_loop(Data, Sql, <<>>, <<>>).

%% list
collect_loop(Data, Head, Format, Tail) when is_list(Data) ->
    collect_list_loop(Data, Head, Format, Tail, <<>>);
%% ets
collect_loop(Tab, Head, Format, Tail) when is_atom(Tab) orelse is_reference(Tab) ->
    ets:safe_fixtable(Tab, true),
    Key = ets:first(Tab),
    Object = ets:lookup(Tab, Key),
    collect_ets_loop(Tab, Key, Object, Head, Format, Tail, <<>>).

%% list
collect_list_loop([], _, _, _, Acc) ->
    Acc;
collect_list_loop([H], Head, Format, Tail, Acc) ->
    %% end of list
    <<Head/binary, Acc/binary, (format(Format, H))/binary, Tail/binary>>;
collect_list_loop([H | T], Head, Format, Tail, Acc) ->
    %% insert delimiter
    collect_list_loop(T, Head, Format, Tail, <<Acc/binary, (format(Format, H))/binary, $,>>).

%% ets
collect_ets_loop(Tab, '$end_of_table', [], _, _, _, <<>>) ->
...
collect_ets_loop(Tab, Key, [H], Head, Format, Tail, <<>>) ->
    Data = format(Format, H),
    %% next
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    %% insert delimiter
    collect_ets_loop(Tab, Next, Object, Head, Format, Tail, Data);
collect_ets_loop(Tab, Key, [H], Head, Format, Tail, Acc) ->
    Data = format(Format, H),
    %% next
    Next = ets:next(Tab, Key),
    Object = ets:lookup(Tab, Next),
    %% insert delimiter
    collect_ets_loop(Tab, Next, Object, Head, Format, Tail, <<Acc/binary, ",", Data/binary>>).

%% @doc save data
-spec save_into(Head :: binary(), Format :: binary(), Tail :: binary(), Data :: list() | ets:tab(), Flag :: pos_integer()) -> NewData :: list() | ok.
save_into(Head, Format, Tail, Data, Flag) ->
    %% tuple sql
    save_into_loop(Head, Format, Tail, Data, fun(A) -> A end, Flag).

%% @doc save data
-spec save_into(Head :: binary(), Format :: binary(), Data :: list() | ets:tab(), Flag :: pos_integer()) -> NewData :: list() | ok.
save_into(Head, Format, Data, Flag) ->
    %% pair sql
    save_into_loop(Head, Format, <<>>, Data, fun(A) -> A end, Flag).

%% @doc save data
-spec save_into( Sql :: binary(), Data :: list() | ets:tab(), Flag :: pos_integer()) -> NewData :: list() | ok.
save_into(Sql, Data, Flag) ->
    %% single sql
    save_into_loop(Sql, <<>>, <<>>, Data, fun(A) -> A end, Flag).

%% list
save_into_loop(Head, Format, Tail, Data, Collect, Flag) when is_list(Data) ->
    save_into_list_loop(Head, Format, Tail, Data, Collect, Flag, <<>>, []);
%% ets
save_into_loop(Head, Format, Tail, Tab, Collect, Flag) when is_atom(Tab) orelse is_reference(Tab) ->
    ets:safe_fixtable(Tab, true),
    Key = ets:first(Tab),
    Object = ets:lookup(Tab, Key),
    save_into_ets_loop(Head, Format, Tail, Tab, Collect, Key, Object, Flag, <<>>).
...
serialize(Value) ->
    <<"'", (quote_string(type:to_binary(Value)))/binary, "'">>.
...
get_auto_increment(Table) ->
    select_one(db:format(<<"SELECT AUTO_INCREMENT FROM information_schema.`TABLES` WHERE `TABLE_SCHEMA` = DATABASE() AND `TABLE_NAME` = '~s'">>, [Table])).
...
set_auto_increment(Table, AutoIncrement) ->
    %% maximize
    query(db:format(<<"ALTER TABLE `~s` AUTO_INCREMENT = ~w">>, [Table, AutoIncrement])).
