...
fill(Table) ->
    try
        ...
        lists:foreach(fun(_) -> db:save_into(<<"INSERT INTO ", "`", Table/binary, "`", "(", (join(Names))/binary, ")", " VALUES ">>, <<"(", (join(lists:duplicate(length(Fill), <<"?">>)))/binary, ")">>, <<>>, [[apply(F, [Table]) || F <- Fill] || <<_:8>> <= binary:copy(<<0>>, 10000)], fun(A) -> A end, 0), erlang:garbage_collect(self()) end, lists:seq(1, Rows)),
