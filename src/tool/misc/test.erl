...
test_protocol(User, Protocol, Binary, Bol, U8, U16, U32, U64, I8, I16, I32, I64, F32, F64, StrString, Bst, List, KeyList) ->
    io:format("~ts~n", pretty:print([User, Protocol, Binary, Bol, U8, U16, U32, U64, I8, I16, I32, I64, F32, F64, StrString, Bst, List, KeyList])).
...
test_collect_list() ->
    ...
    F = fun() -> db:collect(<<"insert into `test` (`a`, `b`, `c`) values ">>, <<"(?, ?, ?)">>, <<" on duplicate key update `type` = VALUES(`type`), `type` = VALUES(`type`), `type` = VALUES(`type`)">>, L, fun(A) -> A end, 0) end,
    timer:tc(F).
...
test_collect_ets() ->
    ...
    L = [{X, randomness:rand(1,100), randomness:rand(1,100), 1} || X <- lists:seq(1, 1000)],
    ets:insert(test, L),
    F = fun() -> db:collect(<<"insert into `test` (`a`, `b`, `c`) values ">>, <<"(?, ?, ?)">>, <<" on duplicate key update `type` = VALUES(`type`), `type` = VALUES(`type`), `type` = VALUES(`type`)">>, test, fun(A) -> A end, 0) end,
    timer:tc(F).

test_collect_into_list() ->
    L = [{X, randomness:rand(1,100), randomness:rand(1,100), 1} || X <- lists:seq(1, 1000)],
    F = fun() -> db:collect(<<"insert into `test` (`a`, `b`, `c`) values ">>, <<"(?, ?, ?)">>, <<" on duplicate key update `type` = VALUES(`type`), `type` = VALUES(`type`), `type` = VALUES(`type`)">>, L, fun(A) -> A end, 4) end,
    timer:tc(F).
...
test_collect_into_ets() ->
    ...
    L = [{X, randomness:rand(1,100), randomness:rand(1,100), 1} || X <- lists:seq(1, 1000)],
    ets:insert(test, L),
    F = fun() -> db:collect(<<"insert into `test` (`a`, `b`, `c`) values ">>, <<"(?, ?, ?)">>, <<" on duplicate key update `type` = VALUES(`type`), `type` = VALUES(`type`), `type` = VALUES(`type`)">>, test, fun(A) -> A end, 4) end,
    timer:tc(F).
