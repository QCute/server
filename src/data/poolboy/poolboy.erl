...
-spec checkout(Pool :: pool()) -> {ok, pid()} | {error, full}.
checkout(Pool) ->
    checkout(Pool, true).

-spec checkout(Pool :: pool(), Block :: boolean()) -> {ok, pid()} | {error, full}.
checkout(Pool, Block) ->
    checkout(Pool, Block, ?TIMEOUT).

-spec checkout(Pool :: pool(), Block :: boolean(), Timeout :: timeout()) -> {ok, pid()} | {error, full}.
checkout(Pool, Block, Timeout) ->
    CRef = make_ref(),
    try
