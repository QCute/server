...
-compile(export_all).
-include("auction.hrl").
-define(INSERT_AUCTION, <<"INSERT INTO `auction` (`auction_id`, `number`, `type`, `start_time`, `end_time`, `from`, `bid_number`, `price`, `seller_list`, `role_id`, `role_name`, `server_id`) VALUES (~w, ~w, ~w, ~w, ~w, '~w', ~w, ~w, '~w', ~w, '~s', ~w)">>).
-define(SELECT_AUCTION, <<"SELECT `unique_id`, `auction_id`, `number`, `type`, `start_time`, `end_time`, `from`, `bid_number`, `price`, `seller_list`, `role_id`, `role_name`, `server_id`, `timer`, 0 AS `flag` FROM `auction`">>).
-define(UPDATE_AUCTION, <<"UPDATE `auction` SET `auction_id` = ~w, `number` = ~w, `type` = ~w, `start_time` = ~w, `end_time` = ~w, `from` = '~w', `bid_number` = ~w, `price` = ~w, `seller_list` = '~w', `role_id` = ~w, `role_name` = '~s', `server_id` = ~w WHERE `unique_id` = ~w">>).
