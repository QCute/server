...
write(19004, [List]) ->
    {ok, protocol:pack(19004, <<(length(List)):16, <<<<Type:16, Rank:64, Key:64, Value:64, Time:32, (byte_size(Name)):16, (Name)/binary, Level:16, Classes:8, Sex:8>> || #rank{type = Type, rank = Rank, key = Key, value = Value, time = Time, name = Name, other = {Level, Classes, Sex}} <- List>>/binary>>)};

write(19005, [List]) ->
    {ok, protocol:pack(19005, <<(length(List)):16, <<<<Type:16, Rank:64, Key:64, Value:64, Time:32, (byte_size(Name)):16, (Name)/binary, Level:16, Classes:8, Sex:8>> || #rank{type = Type, rank = Rank, key = Key, value = Value, time = Time, name = Name, other = {Level, Classes, Sex}} <- List>>/binary>>)};
