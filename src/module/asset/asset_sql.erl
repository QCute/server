...
update(#asset{role_id = RoleId, gold = Gold, silver = Silver, copper = Copper, coin = Coin, exp = Exp}) ->
    db:update(<<"UPDATE `asset` SET `gold` = ?, `silver` = ?, `copper` = ?, `coin` = ?, `exp` = ? WHERE `role_id` = ?">>, [RoleId, Gold, Silver, Copper, Coin, Exp, RoleId]).
