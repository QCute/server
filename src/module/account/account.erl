...
create_check_name(Client, RoleName, ServerId, AccountName, Sex, Classes, Channel, DeviceId, Mac, DeviceType) ->
    ...
    case word:validate(RoleName, [{length, 1, 6}, sensitive, {sql, db:format(<<"SELECT `role_name` FROM `role` WHERE `role_name` = '~s' OR `account_name` = '~s'">>, [RoleName, AccountName])}]) of
        true ->
            start_create(Client, RoleName, ServerId, AccountName, Sex, Classes, Channel, DeviceId, Mac, DeviceType);
        {false, length, _} ->
...
start_create(Client = #client{ip = IP}, RoleName, ServerId, AccountName, Sex, Classes, Channel, DeviceId, Mac, DeviceType) ->
    Now = time:now(),
    Role = #role{
        role_name = RoleName,
        server_id = ServerId,
...
login(Client, RoleId, RoleName, ServerId, AccountName) ->
    case check_interval(Client, RoleId, RoleName, ServerId, AccountName) of
        {ok, NewClient} ->
...
check_interval(Client, RoleId, RoleName, ServerId, AccountName) ->
...
login_check_user(Client, RoleId, RoleName, ServerId, AccountName, ServerClient) ->
    case db:select(db:format(<<"SELECT `type` FROM `role` WHERE `role_id` = ~w AND `account_name` = '~s'">>, [RoleId, AccountName])) of
        [] ->
            %% cannot find role
...
login_check_started(Client = #client{role_pid = RolePid}, RoleId, RoleName, ServerId, AccountName) ->
    case is_pid(RolePid) of
        true ->
            {ok, Client};
        false ->
            start_login(Client, RoleId, RoleName, ServerId, AccountName)
    end.

%% common login
start_login(Client = #client{socket_type = SocketType, socket = Socket, protocol_type = ProtocolType}, RoleId, RoleName, ServerId, AccountName) ->
    %% new login
