...
insert(Sign) ->
    db:insert(<<"INSERT INTO `sign` (`role_id`, `login_day`, `sign_total`, `is_sign_today`) VALUES (:1:, :2:, :3:, :4:)">>, Sign).
...
update(Sign) ->
    db:update(<<"UPDATE `sign` SET `login_day` = :2:, `sign_total` = :3:, `is_sign_today` = :4: WHERE `role_id` = :1:">>, Sign).
