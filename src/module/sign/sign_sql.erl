...
update(#sign{role_id = RoleId, login_day = LoginDay, sign_total = SignTotal, is_sign_today = IsSignToday}) ->
    db:update(<<"UPDATE `sign` SET `login_day` = ?, `sign_total` = ?, `is_sign_today` = ? WHERE `role_id` = ?">>, [RoleId, LoginDay, SignTotal, IsSignToday, RoleId]).
