...
subtract_kv({2, Value}, Attribute = #attribute{power = Power}) ->
    Attribute#attribute{power = Power - Value};
subtract_kv({3, Value}, Attribute = #attribute{dexterity = Dexterity}) ->
    Attribute#attribute{dexterity = Dexterity - Value};
subtract_kv({4, Value}, Attribute = #attribute{vitality = Vitality}) ->
    Attribute#attribute{vitality = Vitality - Value};
subtract_kv({5, Value}, Attribute = #attribute{intellect = Intellect}) ->
    Attribute#attribute{intellect = Intellect - Value};
subtract_kv({6, Value}, Attribute = #attribute{attack = Attack}) ->
    Attribute#attribute{attack = Attack - Value};
subtract_kv({7, Value}, Attribute = #attribute{attack_min = AttackMin}) ->
    Attribute#attribute{attack_min = AttackMin - Value};
subtract_kv({8, Value}, Attribute = #attribute{attack_max = AttackMax}) ->
    Attribute#attribute{attack_max = AttackMax - Value};
subtract_kv({9, Value}, Attribute = #attribute{defense = Defense}) ->
    Attribute#attribute{defense = Defense - Value};
subtract_kv({10, Value}, Attribute = #attribute{total_hp = TotalHp}) ->
    Attribute#attribute{total_hp = TotalHp - Value};
subtract_kv({11, Value}, Attribute = #attribute{hit = Hit}) ->
    Attribute#attribute{hit = Hit - Value};
subtract_kv({12, Value}, Attribute = #attribute{duck = Duck}) ->
    Attribute#attribute{duck = Duck - Value};
subtract_kv({13, Value}, Attribute = #attribute{hit_rate = HitRate}) ->
    Attribute#attribute{hit_rate = HitRate - Value};
subtract_kv({14, Value}, Attribute = #attribute{duck_rate = DuckRate}) ->
    Attribute#attribute{duck_rate = DuckRate - Value};
subtract_kv({15, Value}, Attribute = #attribute{attack_speed = AttackSpeed}) ->
    Attribute#attribute{attack_speed = AttackSpeed - Value};
subtract_kv({16, Value}, Attribute = #attribute{skill_hurt_per = SkillHurtPer}) ->
    Attribute#attribute{skill_hurt_per = SkillHurtPer - Value};
subtract_kv({17, Value}, Attribute = #attribute{skill_hurt = SkillHurt}) ->
    Attribute#attribute{skill_hurt = SkillHurt - Value};
subtract_kv({18, Value}, Attribute = #attribute{hurt_add_per = HurtAddPer}) ->
    Attribute#attribute{hurt_add_per = HurtAddPer - Value};
subtract_kv({19, Value}, Attribute = #attribute{hurt_add_per_4_show = HurtAddPer4Show}) ->
    Attribute#attribute{hurt_add_per_4_show = HurtAddPer4Show - Value};
subtract_kv({20, Value}, Attribute = #attribute{hurt_dec_per = HurtDecPer}) ->
    Attribute#attribute{hurt_dec_per = HurtDecPer - Value};
subtract_kv({21, Value}, Attribute = #attribute{attack_fixed = AttackFixed}) ->
    Attribute#attribute{attack_fixed = AttackFixed - Value};
subtract_kv({22, Value}, Attribute = #attribute{defense_fixed = DefenseFixed}) ->
    Attribute#attribute{defense_fixed = DefenseFixed - Value};
subtract_kv({23, Value}, Attribute = #attribute{ignore_def_rate = IgnoreDefRate}) ->
    Attribute#attribute{ignore_def_rate = IgnoreDefRate - Value};
subtract_kv({24, Value}, Attribute = #attribute{resist_ignore_def = ResistIgnoreDef}) ->
    Attribute#attribute{resist_ignore_def = ResistIgnoreDef - Value};
subtract_kv({25, Value}, Attribute = #attribute{power_hit_rate = PowerHitRate}) ->
    Attribute#attribute{power_hit_rate = PowerHitRate - Value};
subtract_kv({26, Value}, Attribute = #attribute{diligence_rate = DiligenceRate}) ->
    Attribute#attribute{diligence_rate = DiligenceRate - Value};
subtract_kv({27, Value}, Attribute = #attribute{power_hit_add_per = PowerHitAddPer}) ->
    Attribute#attribute{power_hit_add_per = PowerHitAddPer - Value};
subtract_kv({28, Value}, Attribute = #attribute{power_hit_dec_per = PowerHitDecPer}) ->
    Attribute#attribute{power_hit_dec_per = PowerHitDecPer - Value};
subtract_kv({29, Value}, Attribute = #attribute{power_hit_add_fixed = PowerHitAddFixed}) ->
    Attribute#attribute{power_hit_add_fixed = PowerHitAddFixed - Value};
subtract_kv({30, Value}, Attribute = #attribute{power_hit_dec_fixed = PowerHitDecFixed}) ->
    Attribute#attribute{power_hit_dec_fixed = PowerHitDecFixed - Value};
subtract_kv({31, Value}, Attribute = #attribute{move_speed = MoveSpeed}) ->
    Attribute#attribute{move_speed = MoveSpeed - Value};
subtract_kv({32, Value}, Attribute = #attribute{critical_hit_rate = CriticalHitRate}) ->
    Attribute#attribute{critical_hit_rate = CriticalHitRate - Value};
subtract_kv({33, Value}, Attribute = #attribute{resist_critical_hit = ResistCriticalHit}) ->
    Attribute#attribute{resist_critical_hit = ResistCriticalHit - Value};
subtract_kv({34, Value}, Attribute = #attribute{critical_hit_add_per = CriticalHitAddPer}) ->
    Attribute#attribute{critical_hit_add_per = CriticalHitAddPer - Value};
subtract_kv({35, Value}, Attribute = #attribute{critical_hit_dec_per = CriticalHitDecPer}) ->
    Attribute#attribute{critical_hit_dec_per = CriticalHitDecPer - Value};
subtract_kv({36, Value}, Attribute = #attribute{critical_hit_add_fixed = CriticalHitAddFixed}) ->
    Attribute#attribute{critical_hit_add_fixed = CriticalHitAddFixed - Value};
subtract_kv({37, Value}, Attribute = #attribute{critical_hit_dec_fixed = CriticalHitDecFixed}) ->
    Attribute#attribute{critical_hit_dec_fixed = CriticalHitDecFixed - Value};
subtract_kv({38, Value}, Attribute = #attribute{total_mp = TotalMp}) ->
    Attribute#attribute{total_mp = TotalMp - Value};
subtract_kv({39, Value}, Attribute = #attribute{magic_defense = MagicDefense}) ->
    Attribute#attribute{magic_defense = MagicDefense - Value};
subtract_kv({40, Value}, Attribute = #attribute{ignore_strike_hurt_add_per = IgnoreStrikeHurtAddPer}) ->
    Attribute#attribute{ignore_strike_hurt_add_per = IgnoreStrikeHurtAddPer - Value};
subtract_kv({41, Value}, Attribute = #attribute{ignore_strike_hurt_dec_per = IgnoreStrikeHurtDecPer}) ->
    Attribute#attribute{ignore_strike_hurt_dec_per = IgnoreStrikeHurtDecPer - Value};
subtract_kv({42, Value}, Attribute = #attribute{act_hurt_max = ActHurtMax}) ->
    Attribute#attribute{act_hurt_max = ActHurtMax - Value};
subtract_kv({43, Value}, Attribute = #attribute{act_hurt_min = ActHurtMin}) ->
    Attribute#attribute{act_hurt_min = ActHurtMin - Value};
subtract_kv({44, Value}, Attribute = #attribute{target_hurt_max = TargetHurtMax}) ->
    Attribute#attribute{target_hurt_max = TargetHurtMax - Value};
subtract_kv({45, Value}, Attribute = #attribute{target_hurt_min = TargetHurtMin}) ->
    Attribute#attribute{target_hurt_min = TargetHurtMin - Value};
subtract_kv({46, Value}, Attribute = #attribute{paralysis = Paralysis}) ->
    Attribute#attribute{paralysis = Paralysis - Value};
subtract_kv({47, Value}, Attribute = #attribute{resist_paralysis = ResistParalysis}) ->
    Attribute#attribute{resist_paralysis = ResistParalysis - Value};
subtract_kv({48, Value}, Attribute = #attribute{reduce_speed = ReduceSpeed}) ->
    Attribute#attribute{reduce_speed = ReduceSpeed - Value};
subtract_kv({49, Value}, Attribute = #attribute{resist_reduce_speed = ResistReduceSpeed}) ->
    Attribute#attribute{resist_reduce_speed = ResistReduceSpeed - Value};
subtract_kv({50, Value}, Attribute = #attribute{vertigo = Vertigo}) ->
    Attribute#attribute{vertigo = Vertigo - Value};
subtract_kv({51, Value}, Attribute = #attribute{resist_vertigo = ResistVertigo}) ->
    Attribute#attribute{resist_vertigo = ResistVertigo - Value};
subtract_kv({52, Value}, Attribute = #attribute{silence = Silence}) ->
    Attribute#attribute{silence = Silence - Value};
subtract_kv({53, Value}, Attribute = #attribute{resist_silence = ResistSilence}) ->
    Attribute#attribute{resist_silence = ResistSilence - Value};
subtract_kv({54, Value}, Attribute = #attribute{suck_hp = SuckHp}) ->
    Attribute#attribute{suck_hp = SuckHp - Value};
subtract_kv({55, Value}, Attribute = #attribute{kill_mon_exp = KillMonExp}) ->
    Attribute#attribute{kill_mon_exp = KillMonExp - Value};
subtract_kv({56, Value}, Attribute = #attribute{kill_mon_copper = KillMonCopper}) ->
    Attribute#attribute{kill_mon_copper = KillMonCopper - Value};
subtract_kv({57, Value}, Attribute = #attribute{parry_per = ParryPer}) ->
    Attribute#attribute{parry_per = ParryPer - Value};
subtract_kv({58, Value}, Attribute = #attribute{skill_hurt_add_per = SkillHurtAddPer}) ->
    Attribute#attribute{skill_hurt_add_per = SkillHurtAddPer - Value};
subtract_kv({59, Value}, Attribute = #attribute{attack_add_hp_fixed = AttackAddHpFixed}) ->
    Attribute#attribute{attack_add_hp_fixed = AttackAddHpFixed - Value};
subtract_kv({60, Value}, Attribute = #attribute{combo_attack_rate = ComboAttackRate}) ->
    Attribute#attribute{combo_attack_rate = ComboAttackRate - Value};
subtract_kv({61, Value}, Attribute = #attribute{resist_control = ResistControl}) ->
    Attribute#attribute{resist_control = ResistControl - Value};
subtract_kv({62, Value}, Attribute = #attribute{attack_fixed_by_level = AttackFixedByLevel}) ->
    Attribute#attribute{attack_fixed_by_level = AttackFixedByLevel - Value};
subtract_kv({63, Value}, Attribute = #attribute{attack_add_hp_fixed_by_level = AttackAddHpFixedByLevel}) ->
    Attribute#attribute{attack_add_hp_fixed_by_level = AttackAddHpFixedByLevel - Value};
subtract_kv({64, Value}, Attribute = #attribute{attack_add_hp_fixed_only_pvp = AttackAddHpFixedOnlyPvp}) ->
    Attribute#attribute{attack_add_hp_fixed_only_pvp = AttackAddHpFixedOnlyPvp - Value};
subtract_kv({65, Value}, Attribute = #attribute{ack_weapon = AckWeapon}) ->
    Attribute#attribute{ack_weapon = AckWeapon - Value};
subtract_kv({66, Value}, Attribute = #attribute{ack_jewelry = AckJewelry}) ->
    Attribute#attribute{ack_jewelry = AckJewelry - Value};
subtract_kv({67, Value}, Attribute = #attribute{def_armor = DefArmor}) ->
    Attribute#attribute{def_armor = DefArmor - Value};
subtract_kv({68, Value}, Attribute = #attribute{hp_armor = HpArmor}) ->
    Attribute#attribute{hp_armor = HpArmor - Value};
subtract_kv({69, Value}, Attribute = #attribute{ack_elements = AckElements}) ->
    Attribute#attribute{ack_elements = AckElements - Value};
subtract_kv({70, Value}, Attribute = #attribute{def_elements = DefElements}) ->
    Attribute#attribute{def_elements = DefElements - Value};
subtract_kv({71, Value}, Attribute = #attribute{base_hp = BaseHp}) ->
    Attribute#attribute{base_hp = BaseHp - Value};
subtract_kv({72, Value}, Attribute = #attribute{counter_ack_fixed = CounterAckFixed}) ->
    Attribute#attribute{counter_ack_fixed = CounterAckFixed - Value};
subtract_kv({73, Value}, Attribute = #attribute{counter_ack_per = CounterAckPer}) ->
    Attribute#attribute{counter_ack_per = CounterAckPer - Value};
subtract_kv({74, Value}, Attribute = #attribute{ignore_strike_rate = IgnoreStrikeRate}) ->
    Attribute#attribute{ignore_strike_rate = IgnoreStrikeRate - Value};
subtract_kv({75, Value}, Attribute = #attribute{add_hp_per_3 = AddHpPer3}) ->
    Attribute#attribute{add_hp_per_3 = AddHpPer3 - Value};
subtract_kv({76, Value}, Attribute = #attribute{add_att_per_3 = AddAttPer3}) ->
    Attribute#attribute{add_att_per_3 = AddAttPer3 - Value};
subtract_kv({77, Value}, Attribute = #attribute{add_def_per_3 = AddDefPer3}) ->
    Attribute#attribute{add_def_per_3 = AddDefPer3 - Value};
subtract_kv({78, Value}, Attribute = #attribute{add_hp_per_2 = AddHpPer2}) ->
    Attribute#attribute{add_hp_per_2 = AddHpPer2 - Value};
subtract_kv({79, Value}, Attribute = #attribute{add_att_per_2 = AddAttPer2}) ->
    Attribute#attribute{add_att_per_2 = AddAttPer2 - Value};
subtract_kv({80, Value}, Attribute = #attribute{add_def_per_2 = AddDefPer2}) ->
    Attribute#attribute{add_def_per_2 = AddDefPer2 - Value};
subtract_kv({81, Value}, Attribute = #attribute{add_hp_per_1 = AddHpPer1}) ->
    Attribute#attribute{add_hp_per_1 = AddHpPer1 - Value};
subtract_kv({82, Value}, Attribute = #attribute{add_att_per_1 = AddAttPer1}) ->
    Attribute#attribute{add_att_per_1 = AddAttPer1 - Value};
subtract_kv({83, Value}, Attribute = #attribute{add_def_per_1 = AddDefPer1}) ->
    Attribute#attribute{add_def_per_1 = AddDefPer1 - Value};
subtract_kv({84, Value}, Attribute = #attribute{attack_add_hp_per = AttackAddHpPer}) ->
    Attribute#attribute{attack_add_hp_per = AttackAddHpPer - Value};
subtract_kv({85, Value}, Attribute = #attribute{be_attack_add_hp_per = BeAttackAddHpPer}) ->
    Attribute#attribute{be_attack_add_hp_per = BeAttackAddHpPer - Value};
subtract_kv({88, Value}, Attribute = #attribute{is_multiple_hurt = IsMultipleHurt}) ->
    Attribute#attribute{is_multiple_hurt = IsMultipleHurt - Value};
subtract_kv({89, Value}, Attribute = #attribute{passive_add_min_attack = PassiveAddMinAttack}) ->
    Attribute#attribute{passive_add_min_attack = PassiveAddMinAttack - Value};
subtract_kv({90, Value}, Attribute = #attribute{passive_add_duck_by_dex = PassiveAddDuckByDex}) ->
    Attribute#attribute{passive_add_duck_by_dex = PassiveAddDuckByDex - Value};
subtract_kv({91, Value}, Attribute = #attribute{passive_add_attack_by_dex = PassiveAddAttackByDex}) ->
    Attribute#attribute{passive_add_attack_by_dex = PassiveAddAttackByDex - Value};
subtract_kv({92, Value}, Attribute = #attribute{passive_add_def_by_pow = PassiveAddDefByPow}) ->
    Attribute#attribute{passive_add_def_by_pow = PassiveAddDefByPow - Value};
subtract_kv({93, Value}, Attribute = #attribute{passive_add_hp_by_int = PassiveAddHpByInt}) ->
    Attribute#attribute{passive_add_hp_by_int = PassiveAddHpByInt - Value};
subtract_kv({94, Value}, Attribute = #attribute{passive_add_hp_by_per = PassiveAddHpByPer}) ->
    Attribute#attribute{passive_add_hp_by_per = PassiveAddHpByPer - Value};
subtract_kv({95, Value}, Attribute = #attribute{passive_fan_recover_be_hit = PassiveFanRecoverBeHit}) ->
    Attribute#attribute{passive_fan_recover_be_hit = PassiveFanRecoverBeHit - Value};
subtract_kv({96, Value}, Attribute = #attribute{passive_power_hit_must_next = PassivePowerHitMustNext}) ->
    Attribute#attribute{passive_power_hit_must_next = PassivePowerHitMustNext - Value};
subtract_kv({97, Value}, Attribute = #attribute{passive_add_skill_hurt_when_duck = PassiveAddSkillHurtWhenDuck}) ->
    Attribute#attribute{passive_add_skill_hurt_when_duck = PassiveAddSkillHurtWhenDuck - Value};
subtract_kv({98, Value}, Attribute = #attribute{passive_add_counter_ack_by_pow = PassiveAddCounterAckByPow}) ->
    Attribute#attribute{passive_add_counter_ack_by_pow = PassiveAddCounterAckByPow - Value};
subtract_kv({99, Value}, Attribute = #attribute{passive_add_buff_when_low_hp = PassiveAddBuffWhenLowHp}) ->
    Attribute#attribute{passive_add_buff_when_low_hp = PassiveAddBuffWhenLowHp - Value};
subtract_kv({100, Value}, Attribute = #attribute{passive_protect = PassiveProtect}) ->
    Attribute#attribute{passive_protect = PassiveProtect - Value};
subtract_kv({101, Value}, Attribute = #attribute{reborn = Reborn}) ->
    Attribute#attribute{reborn = Reborn - Value};
subtract_kv({102, Value}, Attribute = #attribute{shield_can_boom = ShieldCanBoom}) ->
    Attribute#attribute{shield_can_boom = ShieldCanBoom - Value};
subtract_kv({103, Value}, Attribute = #attribute{use_skill_when_dead = UseSkillWhenDead}) ->
    Attribute#attribute{use_skill_when_dead = UseSkillWhenDead - Value};
subtract_kv({104, Value}, Attribute = #attribute{pet_protect_per = PetProtectPer}) ->
    Attribute#attribute{pet_protect_per = PetProtectPer - Value};
subtract_kv({105, Value}, Attribute = #attribute{pet_dead_boom = PetDeadBoom}) ->
    Attribute#attribute{pet_dead_boom = PetDeadBoom - Value};
subtract_kv({106, Value}, Attribute = #attribute{speed = Speed}) ->
    Attribute#attribute{speed = Speed - Value};
subtract_kv({107, Value}, Attribute = #attribute{hp_max = HpMax}) ->
    Attribute#attribute{hp_max = HpMax - Value};
subtract_kv({108, Value}, Attribute = #attribute{mp_max = MpMax}) ->
    Attribute#attribute{mp_max = MpMax - Value};
subtract_kv({109, Value}, Attribute = #attribute{atk_speed = AtkSpeed}) ->
    Attribute#attribute{atk_speed = AtkSpeed - Value};
subtract_kv({110, Value}, Attribute = #attribute{physic_dmg = PhysicDmg}) ->
    Attribute#attribute{physic_dmg = PhysicDmg - Value};
subtract_kv({111, Value}, Attribute = #attribute{magic_dmg = MagicDmg}) ->
    Attribute#attribute{magic_dmg = MagicDmg - Value};
subtract_kv({112, Value}, Attribute = #attribute{physic_def = PhysicDef}) ->
    Attribute#attribute{physic_def = PhysicDef - Value};
subtract_kv({113, Value}, Attribute = #attribute{magic_def = MagicDef}) ->
    Attribute#attribute{magic_def = MagicDef - Value};
subtract_kv({114, Value}, Attribute = #attribute{critical = Critical}) ->
    Attribute#attribute{critical = Critical - Value};
subtract_kv({115, Value}, Attribute = #attribute{tenacity = Tenacity}) ->
    Attribute#attribute{tenacity = Tenacity - Value};
subtract_kv({116, Value}, Attribute = #attribute{accuracy = Accuracy}) ->
    Attribute#attribute{accuracy = Accuracy - Value};
subtract_kv({117, Value}, Attribute = #attribute{evasion = Evasion}) ->
    Attribute#attribute{evasion = Evasion - Value};
subtract_kv({118, Value}, Attribute = #attribute{holy_dmg = HolyDmg}) ->
    Attribute#attribute{holy_dmg = HolyDmg - Value};
subtract_kv({119, Value}, Attribute = #attribute{critical_dmg = CriticalDmg}) ->
    Attribute#attribute{critical_dmg = CriticalDmg - Value};
subtract_kv({120, Value}, Attribute = #attribute{dmg_ratio = DmgRatio}) ->
    Attribute#attribute{dmg_ratio = DmgRatio - Value};
subtract_kv({121, Value}, Attribute = #attribute{def_ratio = DefRatio}) ->
    Attribute#attribute{def_ratio = DefRatio - Value};
subtract_kv({122, Value}, Attribute = #attribute{enhance_control = EnhanceControl}) ->
    Attribute#attribute{enhance_control = EnhanceControl - Value};
subtract_kv({123, Value}, Attribute = #attribute{anti_control = AntiControl}) ->
    Attribute#attribute{anti_control = AntiControl - Value};
subtract_kv({124, Value}, Attribute = #attribute{escape = Escape}) ->
    Attribute#attribute{escape = Escape - Value};
subtract_kv({125, Value}, Attribute = #attribute{anti_escape = AntiEscape}) ->
    Attribute#attribute{anti_escape = AntiEscape - Value};
subtract_kv({126, Value}, Attribute = #attribute{capture = Capture}) ->
    Attribute#attribute{capture = Capture - Value};
subtract_kv({127, Value}, Attribute = #attribute{physic_def_ratio = PhysicDefRatio}) ->
    Attribute#attribute{physic_def_ratio = PhysicDefRatio - Value};
subtract_kv({128, Value}, Attribute = #attribute{magic_def_ratio = MagicDefRatio}) ->
    Attribute#attribute{magic_def_ratio = MagicDefRatio - Value};
subtract_kv({129, Value}, Attribute = #attribute{physic_accuracy = PhysicAccuracy}) ->
    Attribute#attribute{physic_accuracy = PhysicAccuracy - Value};
subtract_kv({130, Value}, Attribute = #attribute{magic_accuracy = MagicAccuracy}) ->
    Attribute#attribute{magic_accuracy = MagicAccuracy - Value};
subtract_kv({131, Value}, Attribute = #attribute{physic_evasion = PhysicEvasion}) ->
    Attribute#attribute{physic_evasion = PhysicEvasion - Value};
subtract_kv({132, Value}, Attribute = #attribute{magic_evasion = MagicEvasion}) ->
    Attribute#attribute{magic_evasion = MagicEvasion - Value};
subtract_kv({133, Value}, Attribute = #attribute{physic_critical = PhysicCritical}) ->
    Attribute#attribute{physic_critical = PhysicCritical - Value};
subtract_kv({134, Value}, Attribute = #attribute{magic_critical = MagicCritical}) ->
    Attribute#attribute{magic_critical = MagicCritical - Value};
subtract_kv({135, Value}, Attribute = #attribute{physic_tenacity = PhysicTenacity}) ->
    Attribute#attribute{physic_tenacity = PhysicTenacity - Value};
subtract_kv({136, Value}, Attribute = #attribute{magic_tenacity = MagicTenacity}) ->
    Attribute#attribute{magic_tenacity = MagicTenacity - Value};
subtract_kv({137, Value}, Attribute = #attribute{heal_val = HealVal}) ->
    Attribute#attribute{heal_val = HealVal - Value};
subtract_kv({138, Value}, Attribute = #attribute{heal_ratio = HealRatio}) ->
    Attribute#attribute{heal_ratio = HealRatio - Value};
subtract_kv({139, Value}, Attribute = #attribute{eff_heal_ratio = EffHealRatio}) ->
    Attribute#attribute{eff_heal_ratio = EffHealRatio - Value};
subtract_kv({140, Value}, Attribute = #attribute{physic_dmg_ratio = PhysicDmgRatio}) ->
    Attribute#attribute{physic_dmg_ratio = PhysicDmgRatio - Value};
subtract_kv({141, Value}, Attribute = #attribute{magic_dmg_ratio = MagicDmgRatio}) ->
    Attribute#attribute{magic_dmg_ratio = MagicDmgRatio - Value};
subtract_kv({142, Value}, Attribute = #attribute{fc = Fc}) ->
    Attribute#attribute{fc = Fc - Value};
subtract_kv({143, Value}, Attribute = #attribute{strength = Strength}) ->
    Attribute#attribute{strength = Strength - Value};
subtract_kv({144, Value}, Attribute = #attribute{constitution = Constitution}) ->
    Attribute#attribute{constitution = Constitution - Value};
subtract_kv({145, Value}, Attribute = #attribute{magic = Magic}) ->
    Attribute#attribute{magic = Magic - Value};
subtract_kv({146, Value}, Attribute = #attribute{agility = Agility}) ->
    Attribute#attribute{agility = Agility - Value};
subtract_kv({147, Value}, Attribute = #attribute{endurance = Endurance}) ->
    Attribute#attribute{endurance = Endurance - Value};
subtract_kv({148, Value}, Attribute = #attribute{exp_ratio = ExpRatio}) ->
    Attribute#attribute{exp_ratio = ExpRatio - Value};
