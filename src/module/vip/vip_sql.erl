...
update(#vip{role_id = RoleId, vip_level = VipLevel, exp = Exp, expire_time = ExpireTime}) ->
    db:update(<<"UPDATE `vip` SET `vip_level` = ?, `exp` = ?, `expire_time` = ? WHERE `role_id` = ?">>, [RoleId, VipLevel, Exp, ExpireTime, RoleId]).
