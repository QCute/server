...
insert(Package) ->
    db:insert(<<"INSERT INTO `package` (`role_id`, `item_size`, `bag_size`, `body_size`, `store_size`) VALUES (:1:, :2:, :3:, :4:, :5:)">>, Package).
...
update(Package) ->
    db:update(<<"UPDATE `package` SET `item_size` = :2:, `bag_size` = :3:, `body_size` = :4:, `store_size` = :5: WHERE `role_id` = :1:">>, Package).
