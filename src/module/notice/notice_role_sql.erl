...
save(NoticeRoleList) ->
    db:save_into(<<"INSERT INTO `notice_role` (`role_id`, `notice_id`, `read_time`) VALUES">>, <<"(?, ?, ?)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `notice_id` = VALUES(`notice_id`), `read_time` = VALUES(`read_time`)">>, NoticeRoleList, fun(#notice_role{role_id = RoleId, notice_id = NoticeId, read_time = ReadTime}) -> [RoleId, NoticeId, ReadTime] end, #notice_role.flag).
...
-spec update_read(#notice_role{}) -> AffectedRows :: non_neg_integer().
update_read(#notice_role{read_time = ReadTime, role_id = RoleId, notice_id = NoticeId}) ->
    db:update(<<"UPDATE `notice_role` SET `read_time` = ? WHERE `role_id` = ? AND `notice_id` = ?">>, [ReadTime, RoleId, NoticeId]).
