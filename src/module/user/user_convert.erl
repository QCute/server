...
to(#user{role_id = RoleId, role_name = RoleName, pid = Pid, sender_pid = SenderPid, role = #role{map = #map{x = X, y = Y}}}, map) ->
    #fighter{id = RoleId, name = RoleName, pid = Pid, sender_pid = SenderPid, x = X, y = Y, type = ?MAP_OBJECT_ROLE, attribute = #attribute{hp = 13112757923, health = 13112757923}};
to(_Type, _R) ->
