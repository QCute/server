...
save(BubbleList) ->
    db:save(<<"INSERT INTO `bubble` (`role_id`, `bubble_id`, `type`, `expire_time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"">>, BubbleList, fun(#bubble{role_id = RoleId, bubble_id = BubbleId, type = Type, expire_time = ExpireTime}) -> [RoleId, BubbleId, Type, ExpireTime] end, #bubble.flag).

%% @doc select from bubble
-spec select(RoleId :: non_neg_integer()) -> Rows :: [#bubble{}].
select(RoleId) ->
    Data = db:select(<<"SELECT `role_id`, `bubble_id`, `type`, `expire_time` FROM `bubble` WHERE `role_id` = ?">>, [RoleId]),
