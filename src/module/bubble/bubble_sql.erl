...
save(BubbleList) ->
    db:save_into(<<"INSERT INTO `bubble` (`role_id`, `bubble_id`, `type`, `expire_time`) VALUES">>, <<"(:1:, :2:, :3:, :4:)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `bubble_id` = VALUES(`bubble_id`), `type` = VALUES(`type`), `expire_time` = VALUES(`expire_time`)">>, BubbleList, #bubble.flag).
