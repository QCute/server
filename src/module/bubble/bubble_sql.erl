...
save(BubbleList) ->
    db:save_into(<<"INSERT INTO `bubble` (`role_id`, `bubble_id`, `type`, `expire_time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `bubble_id` = VALUES(`bubble_id`), `type` = VALUES(`type`), `expire_time` = VALUES(`expire_time`)">>, BubbleList, fun(#bubble{role_id = RoleId, bubble_id = BubbleId, type = Type, expire_time = ExpireTime}) -> [RoleId, BubbleId, Type, ExpireTime] end, #bubble.flag).
