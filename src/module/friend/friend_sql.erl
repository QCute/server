...
-define(SELECT_JOIN_FRIEND, <<"SELECT IFNULL(`friend`.`role_id`, 0), IFNULL(`friend`.`friend_id`, 0), IFNULL(`friend`.`state`, 0), IFNULL(`friend`.`time`, 0), IFNULL(`role`.`role_name`, ''), IFNULL(`role`.`sex`, 0), IFNULL(`role`.`classes`, 0), IFNULL(`vip`.`vip_level`, 0), IFNULL(`role`.`online`, 0), IFNULL(`friend`.`flag`, 0) FROM `friend`LEFT JOIN `role` ON `friend`.`friend_id` = `role`.`role_id`LEFT JOIN `vip` ON `friend`.`friend_id` = `vip`.`role_id` WHERE `friend`.`role_id` = '~w'">>).
