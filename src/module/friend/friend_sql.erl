...
save(FriendList) ->
    db:save(<<"INSERT INTO `friend` (`role_id`, `friend_role_id`, `relation`, `time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"">>, FriendList, fun(#friend{role_id = RoleId, friend_role_id = FriendRoleId, relation = Relation, time = Time}) -> [RoleId, FriendRoleId, Relation, Time] end, #friend.flag).

%% @doc select from friend
-spec select(FriendRoleId :: non_neg_integer()) -> Rows :: [#friend{}].
select(FriendRoleId) ->
    Data = db:select(<<"SELECT `friend`.`role_id`, `friend`.`friend_role_id`, `role`.`role_name`, `role`.`sex`, `role`.`avatar`, `role`.`classes`, `role`.`level`, `vip`.`vip_level`, `role`.`is_online`, `friend`.`relation`, `friend`.`time` FROM `friend` INNER JOIN `role` ON `role`.`role_id` = `friend`.`role_id` INNER JOIN `vip` ON `vip`.`role_id` = `friend`.`role_id` WHERE `friend`.`role_id` = ?">>, [FriendRoleId]),
