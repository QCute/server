...
-compile(export_all).
-include("friend.hrl").
-define(INSERT_FRIEND, <<"INSERT INTO `friend` (`role_id`, `friend_id`, `relation`, `time`) VALUES (~i~w, ~w, ~i~i~i~i~i~w, ~w~i)">>).
-define(SELECT_FRIEND, <<"SELECT `role_id`, `friend_id`, '' AS `friend_name`, 0 AS `sex`, 0 AS `classes`, 0 AS `vip_level`, 0 AS `is_online`, `relation`, `time`, 0 AS `flag` FROM `friend` WHERE `role_id` = ~w AND `friend_id` = ~w">>).
-define(UPDATE_FRIEND, {<<"UPDATE `friend` SET ~i~i~i~i~i~i~i~i`relation` = ~w, `time` = ~w~i ">>, <<"WHERE `role_id` = ~w AND `friend_id` = ~w">>}).
-define(DELETE_FRIEND, <<"DELETE  FROM `friend` WHERE `role_id` = ~w AND `friend_id` = ~w">>).
-define(INSERT_UPDATE_FRIEND, {<<"INSERT INTO `friend` (`role_id`, `friend_id`, `relation`, `time`) VALUES ">>, <<"(~i~w, ~w, ~i~i~i~i~i~w, ~w~i)">>, <<" ON DUPLICATE KEY UPDATE `relation` = VALUES(`relation`), `time` = VALUES(`time`)">>}).
-define(SELECT_JOIN_FRIEND, <<"SELECT `friend`.`role_id`, `friend`.`friend_id`, IFNULL(`role`.`role_name`, '') AS `friend_name`, IFNULL(`role`.`sex`, 0) AS `sex`, IFNULL(`role`.`classes`, 0) AS `classes`, IFNULL(`vip`.`vip_level`, 0) AS `vip_level`, IFNULL(`role`.`is_online`, 0) AS `is_online`, `friend`.`relation`, `friend`.`time`, IFNULL(`friend`.`flag`, 0) AS `flag` FROM `friend` LEFT JOIN `role` ON `friend`.`friend_id` = `role`.`role_id` LEFT JOIN `vip` ON `friend`.`friend_id` = `vip`.`role_id` WHERE `friend`.`role_id` = ~w AND `friend`.`friend_id` = ~w">>).
-define(SELECT_BY_ROLE_ID, <<"SELECT `role_id`, `friend_id`, '' AS `friend_name`, 0 AS `sex`, 0 AS `classes`, 0 AS `vip_level`, 0 AS `is_online`, `relation`, `time`, 0 AS `flag` FROM `friend` WHERE `role_id` = ~w">>).
