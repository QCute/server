...
decode(30108, _Rest_ = <<_/binary>>) ->
    <<GuildId:64, _GuildIdRest_/binary>> = _Rest_,
    {ok, GuildId};
...
decode(30120, _Rest_ = <<_/binary>>) ->
    <<NoticeLength:16, Notice:NoticeLength/binary, _NoticeRest_/binary>> = _Rest_,
    {ok, Notice};
...
encode(30101, List) ->
    Data30101 = <<(encode_list_30101(<<>>, 0, ets:safe_fixtable(List, true) andalso List, ets:first(List)))/binary>>,
    {ok, <<(byte_size(Data30101)):16, 30101:16, Data30101/binary>>};

encode(30102, List) ->
    Data30102 = <<(encode_list_30102(<<>>, 0, ets:safe_fixtable(List, true) andalso List, ets:first(List)))/binary>>,
    {ok, <<(byte_size(Data30102)):16, 30102:16, Data30102/binary>>};

encode(30103, List) ->
    Data30103 = <<(encode_list_30103(<<>>, 0, ets:safe_fixtable(List, true) andalso List, ets:first(List)))/binary>>,
...
encode(30106, List) ->
    Data30106 = <<(encode_list_30106(<<>>, 0, List))/binary>>,
    {ok, <<(byte_size(Data30106)):16, 30106:16, Data30106/binary>>};

encode(30107, Result) ->
    Data30107 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30107)):16, 30107:16, Data30107/binary>>};

encode(30108, Result) ->
    Data30108 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30108)):16, 30108:16, Data30108/binary>>};

encode(30109, Result) ->
    Data30109 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30109)):16, 30109:16, Data30109/binary>>};

encode(30110, Result) ->
    Data30110 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30110)):16, 30110:16, Data30110/binary>>};

encode(30111, Result) ->
    Data30111 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30111)):16, 30111:16, Data30111/binary>>};

encode(30112, Result) ->
    Data30112 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30112)):16, 30112:16, Data30112/binary>>};

encode(30113, Result) ->
    Data30113 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30113)):16, 30113:16, Data30113/binary>>};

encode(30114, Result) ->
    Data30114 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30114)):16, 30114:16, Data30114/binary>>};

encode(30115, Result) ->
    Data30115 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30115)):16, 30115:16, Data30115/binary>>};

encode(30116, Result) ->
    Data30116 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30116)):16, 30116:16, Data30116/binary>>};

encode(30117, Result) ->
    Data30117 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30117)):16, 30117:16, Data30117/binary>>};

encode(30118, Result) ->
    Data30118 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30118)):16, 30118:16, Data30118/binary>>};

encode(30119, Result) ->
    Data30119 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30119)):16, 30119:16, Data30119/binary>>};

encode(30120, Result) ->
    Data30120 = <<(protocol:text(Result))/binary>>,
    {ok, <<(byte_size(Data30120)):16, 30120:16, Data30120/binary>>};

encode(Protocol, Data) ->
    {error, Protocol, Data}.

encode_list_30101(Acc = <<_/binary>>, Length, Tab, '$end_of_table') ->
    ets:safe_fixtable(Tab, false),
    <<Length:16, Acc/binary>>;
encode_list_30101(Acc = <<_/binary>>, Length, Tab, Key) ->
    case ets:lookup(Tab, Key) of
        [] ->
            encode_list_30101(Acc, Length, Tab, ets:next(Tab, Key));
        [#guild{guild_id = GuildId, guild_name = GuildName, create_time = CreateTime, leader_role_id = LeaderRoleId, leader_name = LeaderName}] ->
            encode_list_30101(<<Acc/binary, GuildId:64, (byte_size(GuildName)):16, (GuildName)/binary, CreateTime:32, LeaderRoleId:64, (byte_size(LeaderName)):16, (LeaderName)/binary>>, Length + 1, Tab, ets:next(Tab, Key))
    end.

encode_list_30102(Acc = <<_/binary>>, Length, Tab, '$end_of_table') ->
    ets:safe_fixtable(Tab, false),
    <<Length:16, Acc/binary>>;
encode_list_30102(Acc = <<_/binary>>, Length, Tab, Key) ->
    case ets:lookup(Tab, Key) of
        [] ->
            encode_list_30102(Acc, Length, Tab, ets:next(Tab, Key));
        [#guild_role{role_id = RoleId, job = Job, join_time = JoinTime, role_name = RoleName, sex = Sex, classes = Classes, vip_level = VipLevel}] ->
            encode_list_30102(<<Acc/binary, RoleId:64, Job:8, JoinTime:32, (byte_size(RoleName)):16, (RoleName)/binary, Sex:8, Classes:8, VipLevel:8>>, Length + 1, Tab, ets:next(Tab, Key))
    end.

encode_list_30103(Acc = <<_/binary>>, Length, Tab, '$end_of_table') ->
    ets:safe_fixtable(Tab, false),
    <<Length:16, Acc/binary>>;
encode_list_30103(Acc = <<_/binary>>, Length, Tab, Key) ->
    case ets:lookup(Tab, Key) of
        [] ->
            encode_list_30103(Acc, Length, Tab, ets:next(Tab, Key));
        [#guild_apply{role_id = RoleId, apply_time = ApplyTime, role_name = RoleName, sex = Sex, classes = Classes, vip_level = VipLevel}] ->
            encode_list_30103(<<Acc/binary, RoleId:64, ApplyTime:32, (byte_size(RoleName)):16, (RoleName)/binary, Sex:8, Classes:8, VipLevel:8>>, Length + 1, Tab, ets:next(Tab, Key))
    end.

encode_list_30106(Acc = <<_/binary>>, Length, []) ->
    <<Length:16, Acc/binary>>;
encode_list_30106(Acc = <<_/binary>>, Length, [#guild_apply{guild_id = GuildId, apply_time = ApplyTime, guild_name = GuildName} | List]) ->
    encode_list_30106(<<Acc/binary, GuildId:64, ApplyTime:32, (byte_size(GuildName)):16, (GuildName)/binary>>, Length + 1, List).
