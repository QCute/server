...
-define(SELECT_JOIN_GUILD_PLAYER, "SELECT IFNULL(`guild`.`guild_id`, 0), IFNULL(`player`.`id`, 0), `guild_player`.`job`, `guild_player`.`join_time`, `guild_player`.`leave_time`, IFNULL(`guild`.`guild_name`, ''), IFNULL(`player`.`name`, ''), `guild_player`.`player_pid`, `guild_player`.`player_sender_pid`, `guild_player`.`extra` FROM `guild_player`  LEFT JOIN `guild` ON `guild_player`.`guild_id` = `guild`.`guild_id` LEFT JOIN `player` ON `guild_player`.`player_id` = `player`.`id` ").
