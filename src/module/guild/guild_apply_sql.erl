...
-define(SELECT_JOIN_GUILD_APPLY, <<"SELECT IFNULL(`guild_apply`.`guild_id`, 0), IFNULL(`guild_apply`.`role_id`, 0), IFNULL(`guild`.`guild_name`, ''), IFNULL(`role`.`role_name`, ''), IFNULL(`role`.`sex`, 0), IFNULL(`role`.`classes`, 0), IFNULL(`vip`.`vip_level`, 0) FROM `guild_apply`LEFT JOIN `guild` ON `guild_apply`.`guild_id` = `guild`.`guild_id`LEFT JOIN `role` ON `guild_apply`.`role_id` = `role`.`role_id`LEFT JOIN `vip` ON `guild_apply`.`role_id` = `vip`.`role_id`">>).
