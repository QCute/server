...
save(GuildApplyList) ->
    db:save_into(<<"INSERT INTO `guild_apply` (`guild_id`, `role_id`, `apply_time`) VALUES">>, <<"(?, ?, ?)">>, <<"ON DUPLICATE KEY UPDATE `guild_id` = VALUES(`guild_id`), `role_id` = VALUES(`role_id`), `apply_time` = VALUES(`apply_time`)">>, GuildApplyList, fun(#guild_apply{guild_id = GuildId, role_id = RoleId, apply_time = ApplyTime}) -> [GuildId, RoleId, ApplyTime] end, #guild_apply.flag).

%% @doc select from guild_apply
-spec select() -> Rows :: [#guild_apply{}].
select() ->
    Data = db:select(<<"SELECT `guild_apply`.`guild_id`, `guild_apply`.`role_id`, `guild_apply`.`apply_time`, `guild`.`guild_name`, `role`.`role_name`, `role`.`sex`, `role`.`avatar`, `role`.`classes`, `role`.`level`, `vip`.`vip_level`, `guild_apply`.`flag` FROM `guild_apply` INNER JOIN `guild` ON `guild`.`guild_id` = `guild_apply`.`guild_id` INNER JOIN `role` ON `role`.`role_id` = `guild_apply`.`role_id` INNER JOIN `vip` ON `vip`.`role_id` = `guild_apply`.`role_id`">>, []),
