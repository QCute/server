...
-compile(export_all).
-include("guild.hrl").
-define(INSERT_GUILD_APPLY, <<"INSERT INTO `guild_apply` (`guild_id`, `role_id`, `apply_time`) VALUES (~i~w, ~w, ~w~i~i~i~i~i~i~i)">>).
...
-define(UPDATE_GUILD_APPLY, {<<"UPDATE `guild_apply` SET ~i~i~i`apply_time` = ~w~i~i~i~i~i~i~i ">>, <<"WHERE `guild_id` = ~w AND `role_id` = ~w">>}).
-define(DELETE_GUILD_APPLY, <<"DELETE  FROM `guild_apply` WHERE `guild_id` = ~w AND `role_id` = ~w">>).
-define(INSERT_UPDATE_GUILD_APPLY, {<<"INSERT INTO `guild_apply` (`guild_id`, `role_id`, `apply_time`) VALUES ">>, <<"(~i~w, ~w, ~w~i~i~i~i~i~i~i)">>, <<" ON DUPLICATE KEY UPDATE `apply_time` = VALUES(`apply_time`)">>}).
...
%% @doc insert_update
insert_update(Data) ->
    {Sql, NewData} = parser:collect_into(Data, ?INSERT_UPDATE_GUILD_APPLY, #guild_apply.flag),
    db:insert(Sql),
    NewData.
