...
-define(SELECT_JOIN_GUILD_ROLE, <<"SELECT IFNULL(`guild`.`guild_id`, 0), IFNULL(`role`.`role_id`, 0), IFNULL(`guild_role`.`job`, 0), IFNULL(`guild_role`.`join_time`, 0), IFNULL(`guild_role`.`leave_time`, 0), IFNULL(`guild`.`guild_name`, ''), IFNULL(`role`.`role_name`, ''), IFNULL(`role`.`sex`, 0), IFNULL(`role`.`classes`, 0), IFNULL(`vip`.`vip_level`, 0), IFNULL(`guild_role`.`flag`, 0) FROM `guild_role`LEFT JOIN `guild` ON `guild_role`.`guild_id` = `guild`.`guild_id`LEFT JOIN `role` ON `guild_role`.`role_id` = `role`.`role_id`LEFT JOIN `vip` ON `guild_role`.`role_id` = `vip`.`role_id`">>).
