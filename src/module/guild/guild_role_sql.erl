...
-define(SELECT_JOIN_GUILD_ROLE, <<"SELECT `guild`.`guild_id`, `role`.`role_id`, `guild_role`.`job`, `guild_role`.`join_time`, `guild_role`.`leave_time`, `guild`.`guild_name`, `role`.`role_name`, `guild_role`.`role_pid`, `guild_role`.`role_sender_pid`, `guild_role`.`flag` FROM `guild_role`LEFT JOIN `guild` ON `guild_role`.`guild_id` = `guild`.`guild_id`LEFT JOIN `role` ON `guild_role`.`role_id` = `role`.`role_id`">>).
