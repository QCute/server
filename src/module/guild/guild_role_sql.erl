...
-define(SELECT_GUILD_ROLE, <<"SELECT `guild_id`, `role_id`, `job`, `wealth`, `join_time`, `leave_time`, `guild_name`, `role_name`, `sex`, `classes`, `level`, `vip_level`, 0 AS `flag` FROM `guild_role`">>).
...
-define(SELECT_JOIN_GUILD_ROLE, <<"SELECT `guild`.`guild_id`, `role`.`role_id`, `guild_role`.`job`, `guild_role`.`wealth`, `guild_role`.`join_time`, `guild_role`.`leave_time`, IFNULL(`guild`.`guild_name`, '') AS `guild_name`, IFNULL(`role`.`role_name`, '') AS `role_name`, IFNULL(`role`.`sex`, 0) AS `sex`, IFNULL(`role`.`classes`, 0) AS `classes`, IFNULL(`role`.`level`, 0) AS `level`, IFNULL(`vip`.`vip_level`, 0) AS `vip_level`, 0 AS `flag` FROM `guild_role` LEFT JOIN `guild` ON `guild_role`.`guild_id` = `guild`.`guild_id` LEFT JOIN `role` ON `guild_role`.`role_id` = `role`.`role_id` LEFT JOIN `vip` ON `guild_role`.`role_id` = `vip`.`role_id`">>).
