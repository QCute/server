...
save(GuildList) ->
    db:save(<<"INSERT INTO `guild` (`guild_id`, `guild_name`, `exp`, `wealth`, `level`, `create_time`, `notice`, `leader_role_id`) VALUES">>, <<"(?, ?, ?, ?, ?, ?, ?, ?)">>, <<"">>, GuildList, fun(#guild{guild_id = GuildId, guild_name = GuildName, exp = Exp, wealth = Wealth, level = Level, create_time = CreateTime, notice = Notice, leader_role_id = LeaderRoleId}) -> [GuildId, GuildName, Exp, Wealth, Level, CreateTime, Notice, LeaderRoleId] end, #guild.flag).

%% @doc select from guild
-spec select() -> Rows :: [#guild{}].
select() ->
    Data = db:select(<<"SELECT `guild`.`guild_id`, `guild`.`guild_name`, `guild`.`exp`, `guild`.`wealth`, `guild`.`level`, `guild`.`create_time`, `guild`.`notice`, `guild`.`leader_role_id`, `role`.`role_name`, `role`.`sex`, `role`.`avatar`, `role`.`classes`, `role`.`level`, `vip`.`vip_level` FROM `guild` INNER JOIN `role` ON `role`.`role_id` = `guild`.`leader_role_id` INNER JOIN `vip` ON `vip`.`role_id` = `guild`.`leader_role_id`">>, []),
