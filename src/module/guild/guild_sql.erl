...
update_name(Guild) ->
    db:update(<<"UPDATE `guild` SET `guild_name` = :2: WHERE `guild_id` = :1:">>, Guild).

%% @doc update into guild
-spec update_notice(Guild :: #guild{}) -> AffectedRows :: non_neg_integer().
update_notice(Guild) ->
    db:update(<<"UPDATE `guild` SET `notice` = :7: WHERE `guild_id` = :1:">>, Guild).
