...
broadcast(Data) ->
    parser:for(fun(Pid) -> player_sender:send(Pid, Data) end, ?ONLINE, #online.pid).
-spec broadcast(Data :: binary(), ExceptId :: non_neg_integer()) -> ok.
broadcast(Data, ExceptId) ->
    parser:for(fun([#online{id = Id, pid_sender = Pid}]) -> Id =/= ExceptId andalso player_sender:send(Pid, Data) == ok end, ?ONLINE).
...
-spec stop_all() -> ok.
stop_all() ->
    parser:for(fun(Pid) -> gen_server:cast(Pid, {'stop', server_update}) end, ?ONLINE, #online.pid),
    ok.
