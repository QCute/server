...
-define(UPDATE_ROLE, {<<"UPDATE `role` SET ~i~i`role_name` = '~s', `server_id` = ~w, `account_name` = '~s', `origin_server_id` = ~w, `type` = ~w, `status` = ~w, `sex` = ~w, `avatar` = ~w, `classes` = ~w, `level` = ~w, `is_online` = ~w, `register_time` = ~w, `login_time` = ~w, `logout_time` = ~w, `first_recharge_time` = ~w, `last_recharge_time` = ~w, `recharge_total` = ~w, `item_size` = ~w, `bag_size` = ~w, `store_size` = ~w, `map` = '~w', `channel` = '~s', `device_id` = '~s', `device_type` = '~s', `mac` = '~s', `ip` = '~s' ">>, <<"WHERE `role_id` = ~w">>}).
