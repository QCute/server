...
-define(INSERT_ROLE, <<"INSERT INTO `role` (`role_name`, `server_id`, `account_name`, `origin_server_id`, `type`, `status`, `sex`, `avatar`, `classes`, `level`, `is_online`, `register_time`, `login_time`, `logout_time`, `world_chat_time`, `guild_chat_time`, `first_recharge_time`, `last_recharge_time`, `recharge_total`, `item_size`, `bag_size`, `store_size`, `map`, `channel`, `device_id`, `device_type`, `mac`, `ip`) VALUES (~i~i'~s', ~w, '~s', ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, ~w, '~w', '~s', '~s', '~s', '~s', '~s')">>).
...
-define(UPDATE_ROLE, {<<"UPDATE `role` SET ~i~i`role_name` = '~s', `server_id` = ~w, `account_name` = '~s', `origin_server_id` = ~w, `type` = ~w, `status` = ~w, `sex` = ~w, `avatar` = ~w, `classes` = ~w, `level` = ~w, `is_online` = ~w, `register_time` = ~w, `login_time` = ~w, `logout_time` = ~w, `world_chat_time` = ~w, `guild_chat_time` = ~w, `first_recharge_time` = ~w, `last_recharge_time` = ~w, `recharge_total` = ~w, `item_size` = ~w, `bag_size` = ~w, `store_size` = ~w, `map` = '~w', `channel` = '~s', `device_id` = '~s', `device_type` = '~s', `mac` = '~s', `ip` = '~s' ">>, <<"WHERE `role_id` = ~w">>}).
