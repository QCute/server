...
save(TitleList) ->
    db:save_into(<<"INSERT INTO `title` (`role_id`, `title_id`, `type`, `expire_time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `title_id` = VALUES(`title_id`), `type` = VALUES(`type`), `expire_time` = VALUES(`expire_time`)">>, TitleList, fun(#title{role_id = RoleId, title_id = TitleId, type = Type, expire_time = ExpireTime}) -> [RoleId, TitleId, Type, ExpireTime] end, #title.flag).
