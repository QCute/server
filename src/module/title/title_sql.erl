...
save(TitleList) ->
    db:save(<<"INSERT INTO `title` (`role_id`, `title_id`, `type`, `expire_time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"">>, TitleList, fun(#title{role_id = RoleId, title_id = TitleId, type = Type, expire_time = ExpireTime}) -> [RoleId, TitleId, Type, ExpireTime] end, #title.flag).

%% @doc select from title
-spec select(RoleId :: non_neg_integer()) -> Rows :: [#title{}].
select(RoleId) ->
    Data = db:select(<<"SELECT `role_id`, `title_id`, `type`, `expire_time` FROM `title` WHERE `role_id` = ?">>, [RoleId]),
    parser:convert(Data, title).

%% @doc select from title
-spec select_by_title_id(TitleId :: non_neg_integer()) -> Rows :: [#title{}].
select_by_title_id(TitleId) ->
    Data = db:select(<<"SELECT `role_id`, `title_id`, `type`, `expire_time` FROM `title` WHERE `title_id` = ?">>, [TitleId]),
    parser:convert(Data, title).

%% @doc update into title
-spec update_role_id(#title{}, ByRoleId :: non_neg_integer(), ByTitleId :: non_neg_integer()) -> AffectedRows :: non_neg_integer().
