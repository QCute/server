...
save(FashionList) ->
    db:save(<<"INSERT INTO `fashion` (`role_id`, `fashion_id`, `type`, `expire_time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"">>, FashionList, fun(#fashion{role_id = RoleId, fashion_id = FashionId, type = Type, expire_time = ExpireTime}) -> [RoleId, FashionId, Type, ExpireTime] end, #fashion.flag).

%% @doc select from fashion
-spec select(RoleId :: non_neg_integer()) -> Rows :: [#fashion{}].
select(RoleId) ->
    Data = db:select(<<"SELECT `role_id`, `fashion_id`, `type`, `expire_time` FROM `fashion` WHERE `role_id` = ?">>, [RoleId]),
...
select_by_fashion_id(FashionId) ->
    Data = db:select(<<"SELECT `role_id`, `fashion_id`, `type`, `expire_time` FROM `fashion` WHERE `fashion_id` = ?">>, [FashionId]),
    parser:convert(Data, fashion).

%% @doc update into fashion
-spec update_role_id(#fashion{}, ByRoleId :: non_neg_integer(), ByFashionId :: non_neg_integer()) -> AffectedRows :: non_neg_integer().
