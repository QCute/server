...
save(FashionList) ->
    db:save_into(<<"INSERT INTO `fashion` (`role_id`, `fashion_id`, `type`, `expire_time`) VALUES">>, <<"(:1:, :2:, :3:, :4:)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `fashion_id` = VALUES(`fashion_id`), `type` = VALUES(`type`), `expire_time` = VALUES(`expire_time`)">>, FashionList, #fashion.flag).
