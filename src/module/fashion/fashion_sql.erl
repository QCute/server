...
save(FashionList) ->
    db:save_into(<<"INSERT INTO `fashion` (`role_id`, `fashion_id`, `type`, `expire_time`) VALUES">>, <<"(?, ?, ?, ?)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `fashion_id` = VALUES(`fashion_id`), `type` = VALUES(`type`), `expire_time` = VALUES(`expire_time`)">>, FashionList, fun(#fashion{role_id = RoleId, fashion_id = FashionId, type = Type, expire_time = ExpireTime}) -> [RoleId, FashionId, Type, ExpireTime] end, #fashion.flag).
