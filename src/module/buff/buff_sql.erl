...
save(BuffList) ->
    db:save_into(<<"INSERT INTO `buff` (`role_id`, `buff_id`, `expire_time`, `overlap`) VALUES">>, <<"(:1:, :2:, :3:, :4:)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `buff_id` = VALUES(`buff_id`), `expire_time` = VALUES(`expire_time`), `overlap` = VALUES(`overlap`)">>, BuffList, #buff.flag).
