...
-define(UPDATE_LUCKY_MONEY_ROLE, {<<"UPDATE `lucky_money_role` SET ~i~i`server_id` = ~w, ~i`role_name` = '~s', `guild_id` = ~w, `guild_name` = '~s', `gold` = ~w, `time` = ~w~i ">>, <<"WHERE `lucky_money_id` = ~w AND `role_id` = ~w">>}).
-define(DELETE_LUCKY_MONEY_ROLE, <<"DELETE  FROM `lucky_money_role` WHERE `lucky_money_id` = ~w AND `role_id` = ~w">>).
...
-define(DELETE_BY_LUCKY_MONEY_ID, <<"DELETE FROM `lucky_money_role` WHERE `lucky_money_id` = ~w">>).
...
update(LuckyMoneyRole) ->
    Sql = <<(parser:format(element(1, ?UPDATE_LUCKY_MONEY_ROLE), LuckyMoneyRole))/binary, (parser:format(element(2, ?UPDATE_LUCKY_MONEY_ROLE), [LuckyMoneyRole#lucky_money_role.lucky_money_id, LuckyMoneyRole#lucky_money_role.role_id]))/binary>>,
    db:update(Sql).

%% @doc delete
delete(LuckyMoneyId, RoleId) ->
    Sql = parser:format(?DELETE_LUCKY_MONEY_ROLE, [LuckyMoneyId, RoleId]),
    db:delete(Sql).
...
%% @doc delete
delete_by_lucky_money_id(LuckyMoneyId) ->
    Sql = parser:format(?DELETE_BY_LUCKY_MONEY_ID, [LuckyMoneyId]),
    db:delete(Sql).
