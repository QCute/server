...
trigger(User, #event{name = save}) ->
    ...
    DigestUser = digest:on_save(DeviceUser),
    DungeonUser = dungeon:on_save(DigestUser),
...
trigger(User, #event{name = load}) ->
    ...
    DigestUser = digest:on_load(DeviceUser),
    DungeonUser = dungeon:on_load(DigestUser),
...
trigger(User, #event{name = create}) ->
    DeviceUser = device:on_create(User),
    digest:on_create(DeviceUser);
