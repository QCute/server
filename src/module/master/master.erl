...
dispatch(State, Http = #http{uri = <<"/set_role_refuse">>}, Data) ->
    ...
    db:query(db:format(<<"UPDATE `role` SET `type` = ~w WHERE `role_id` IN (~s)">>, [?SERVER_STATE_FORBIDDEN, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_normal">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `type` = ~w WHERE `role_id` IN (~s)">>, [?SERVER_STATE_NORMAL, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_insider">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `type` = ~w WHERE `role_id` IN (~s)">>, [?SERVER_STATE_INSIDER, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_master">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `type` = ~w WHERE `role_id` IN (~s)">>, [?SERVER_STATE_MASTER, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_chat_unlimited">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `status` = `status` WHERE `role_id` IN (~s)">>, [?CHAT_STATE_UNLIMITED, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_chat_silent">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `status` = `status` | ~w WHERE `role_id` IN (~s)">>, [?CHAT_STATE_SILENT, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_chat_silent_world">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `status` = `status` | ~w WHERE `role_id` IN (~s)">>, [?CHAT_STATE_SILENT_WORLD, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_chat_silent_guild">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `status` = `status` | ~w WHERE `role_id` IN (~s)">>, [?CHAT_STATE_SILENT_GUILD, db:join(RoleIdList, <<"~w">>)])),
...
dispatch(State, Http = #http{uri = <<"/set_role_chat_silent_private">>}, Data) ->
    RoleIdList = json:get(<<"role_id">>, Data, []),
    db:query(db:format(<<"UPDATE `role` SET `status` = `status` | ~w WHERE `role_id` IN (~s)">>, [?CHAT_STATE_SILENT_PRIVATE, db:join(RoleIdList, <<"~w">>)])),
