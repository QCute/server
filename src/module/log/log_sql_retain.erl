...
sql() ->
    [
        ...
        {<<"DELETE FROM `asset_produce_log` WHERE `time` < ~w LIMIT 1000 RETURNING *">>, {<<"REPLACE INTO `asset_produce_log` (`id`, `role_id`, `asset`, `asset_id`, `operation`, `from`, `time`) VALUES ">>, <<"(~w, ~w, '~w', ~w, '~w', '~w', ~w)">>, <<";">>}, 2592000},
        {<<"DELETE FROM `asset_consume_log` WHERE `time` < ~w LIMIT 1000 RETURNING *">>, {<<"REPLACE INTO `asset_consume_log` (`id`, `role_id`, `asset`, `asset_id`, `operation`, `from`, `time`) VALUES ">>, <<"(~w, ~w, '~w', ~w, '~w', '~w', ~w)">>, <<";">>}, 2592000}
    ].
