...
sql() ->
    [
        ...
        {<<"SELECT * FROM `total_login_log` WHERE `time` < ~w LIMIT 1000">>, {<<"REPLACE INTO `total_login_log` (`id`, `total`, `hour_list`, `time`) VALUES ">>, <<"(~w, ~w, '~w', ~w)">>, <<";">>}, {<<"DELETE FROM `total_login_log` WHERE `id` IN (">>, <<"~w">>, <<")">>}, 2592000},
        {<<"SELECT * FROM `login_log` WHERE `time` < ~w LIMIT 1000">>, {<<"REPLACE INTO `login_log` (`id`, `role_id`, `ip`, `device_id`, `login_time`, `online_time`, `time`) VALUES ">>, <<"(~w, ~w, '~w', '~w', ~w, ~w, ~w)">>, <<";">>}, {<<"DELETE FROM `login_log` WHERE `id` IN (">>, <<"~w">>, <<")">>}, 2592000},
        ...
        {<<"SELECT * FROM `item_produce_log` WHERE `time` < ~w LIMIT 1000">>, {<<"REPLACE INTO `item_produce_log` (`id`, `role_id`, `item_id`, `operation`, `source`, `time`) VALUES ">>, <<"(~w, ~w, ~w, '~w', '~w', ~w)">>, <<";">>}, {<<"DELETE FROM `item_produce_log` WHERE `id` IN (">>, <<"~w">>, <<")">>}, 2592000},
        {<<"SELECT * FROM `item_consume_log` WHERE `time` < ~w LIMIT 1000">>, {<<"REPLACE INTO `item_consume_log` (`id`, `role_id`, `item_id`, `operation`, `source`, `time`) VALUES ">>, <<"(~w, ~w, ~w, '~w', '~w', ~w)">>, <<";">>}, {<<"DELETE FROM `item_consume_log` WHERE `id` IN (">>, <<"~w">>, <<")">>}, 2592000},
