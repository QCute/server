...
main([]) ->
    ...
    List = [io_lib:format("{\"file\":\"~s\",\"description\":\"~ts\"}", [File, Description]) || {File, Description, _} <- js()],
    io:format("[~n~ts~n]~n", [string:join(List, ",\n")]);
main(Keys) ->
    io:setopts([{encoding, unicode}]),
    io:setopts(standard_error, [{encoding, unicode}]),
    code:add_path(filename:dirname(escript:script_name()) ++ "/../../../beam/"),
    Js = [X || X <- js(), lists:member(filename:basename(element(1, X), ".js"), Keys)],
    try
...
js() ->
    [
        {"script/make/js/data/testData.js", "测试配置",
            [
                %% key -> value
        ...
        {"script/make/js/data/parameterData.js", "自定义参数配置",
            [
