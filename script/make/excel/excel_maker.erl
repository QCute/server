...
parse_table(Table) ->
    ...
    SheetData = #{name => SheetName, state => "", protected => false, data => [ColumnName, ColumnComment | SourceData], meta => MetaList},

    %% validation sheet
    ValidationSheet = maps:from_list([{Name, #{name => Name, state => "hidden", protected => true, data => Data, meta => []}} || #{type := Type, sheet := Name, validation := Data} <- MetaList, Type == enum]),

    %% all sheet data
    %% convert unicode binary list to binary
    #{file => SheetName, sheet => ValidationSheet#{SheetName => SheetData}}.
...
parse_validation_field([#field{name = Name, format = <<"enum">>, type = <<"enum", Type/binary>>} | T], Table, Foreign, Key, Value, Index, List) ->
    ...
    Item = #{
        ...
        sheet => lists:concat([Table, "-", binary_to_list(Name)]),
        type => enum,
        alias => Alias,
        select => Select,
        join => Join,
...
import_table(Name, #{data := [Head, _ | SheetData]}) ->
    ...
    %% connect database
    maker:connect_database(),
    Sql = lists:concat(["SELECT `TABLE_NAME` FROM information_schema.`TABLES` WHERE `TABLE_SCHEMA` = DATABASE() AND `TABLE_COMMENT` = '", Name, "'"]),
    case db:select(unicode:characters_to_binary(Sql)) of
        [[Table]] ->
            ...
            db:save(<<"INSERT INTO ", "`", Table/binary, "` ", Names/binary, " VALUES ">>, Format, <<"ON DUPLICATE KEY UPDATE ", Updates/binary>>, SheetData, fun(A) -> A end, 0),
            ok;
        [] ->
