...
parse_table(Table) ->
    ...
    #{data := TableData, meta := TableMeta} = split([ColumnName, ColumnComment | SourceData], Meta),
...
parse_validation_field([#field{name = Name, format = Format = <<"enum">>, type = <<"enum", Type/binary>>} | T], Table, Foreign, Key, Value, Index, Map) ->
    ...
    Select = lists:concat([
        "IFNULL((`", Alias, "`.`", Value, "`), `", type:to_list(Name), "`) AS `", type:to_list(Name), "`"
    ]),
...
split_column([ColumnData | T], Row, Column, Map, List) ->
    case maps:get(Column, Map) of
        #{type := varchar, max := Max} when Row == 1 orelse Row == 2 ->
            Padding = lists:duplicate(Max - 1, []),
            split_column(T, Row, Column + 1, Map, lists:merge(Padding, [ColumnData | List]));
        #{type := varchar, max := Max} ->
            ...
            split_column(T, Row, Column + 1, Map, lists:merge(lists:merge(Sub, Padding), List));
        _ ->
...
%% normal table
import_table(Name, #{data := [Head, _ | SheetData]}) ->
    ...
    db:save_into(<<"INSERT INTO ", "`", (list_to_binary(TableName))/binary, "` ", Names/binary, " VALUES ">>, Format, <<"ON DUPLICATE KEY UPDATE ", Updates/binary>>, SheetData, fun(A) -> A end, 0),
    ok.
...
join(Sheet = #{data := [Head | _] = TableData}) ->
...
find_join_column(["" | T], Index, 0, List) ->
    find_join_column(T, Index + 1, Index - 1, List);

find_join_column(["" | T], Index, Column, List) ->
...
join_column([#{column := Column, length := Length} | T], Row, Column, RowData, List) when Row == 1 orelse Row == 2 ->
    {[Head | _], SubRowData} = lists:split(Length, RowData),
    join_column(T, Row, Column + 1, SubRowData, [Head | List]);

join_column([#{column := Column, length := Length} | T], Row, Column, RowData, List) when Row =/= 1 andalso Row =/= 2 ->
