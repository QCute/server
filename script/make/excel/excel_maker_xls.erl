...
export_loop([{Name, Dir = #{}} | T], Xls, Path, Files) ->
    ...
    case erlang:function_exported(?MODULE, PathNameFunction, 1) of
        true ->
            ...
            SubFileList = [{SubName, unicode:characters_to_binary(xmerl:export_simple([SubData], xmerl_xml))} || #{name := SubName, data := SubData} <- SubList],
...
export_loop([{Name, <<>>} | T], Xls, Path, Files) ->
    ...
    case erlang:function_exported(?MODULE, PathNameFunction, 1) of
        true ->
            ...
            Binary = unicode:characters_to_binary(xmerl:export_simple([Data], xmerl_xml)),
...
'xl/workbook.xml'(Sheet) ->
    ...
    SheetNames = make_sheet_name(maps:to_list(Sheet), 1, []),
...
make_sheet_name([], _, List) ->
    List;
make_sheet_name([{Name, #{state := State}} | SheetName], Index, List) ->
    Element = #xmlElement{
        name = 'sheet',
        attributes = [
...
'xl/worksheets'(Sheet) ->
    make_work_sheet(maps:to_list(Sheet), 1, []).


make_work_sheet([], _, List) ->
    lists:reverse(List);
make_work_sheet([{_, Sheet = #{data := Data, meta := Meta}} | T], Index, List) ->

    SheetData = make_sheet_row(Data, 1, []),

    DataValidation = make_sheet_validation(Meta, []),
...
make_sheet_row([], _, List) ->
    lists:reverse(List);
make_sheet_row([Row | RowData], RowIndex, List) ->
    ...
    Element = #xmlElement{
        ...
        attributes = [
            ...
            #xmlAttribute{name = 'spans', value = "1:15"}
        ],
        content = Columns
    },
    make_sheet_row(RowData, RowIndex + 1, [Element | List]).
...
make_sheet_validation([#{type := enum, column := Column, sheet := Sheet} | T], List) ->
    Element = #xmlElement{
        name = 'dataValidation',
        attributes = [
            #xmlAttribute{
...
read_share_string(Files, [#xmlElement{attributes = Attributes}]) ->
    ...
    Data = maps:get(File, Files),
    {Xml, []} = xmerl_scan:string(Data),
    StringElement = xmerl_xpath:string("//sst//si//t", Xml),
    read_share_string_text(StringElement, 1, []).
...
restore(_, [], Map) ->
    Map;
restore(Book, [{SheetName, Sheet = #{data := [Column, Comment | RowData], validation := ValidationData}} | T], Map) ->
