...
make_custom_property(XmlData, [#{name := Name, meta := ValidationData} | T], List) ->
...
make_origin_validation(XmlData, SheetName, [_ | T], List) ->
    make_origin_validation(XmlData, SheetName, T, List).
...
make_sheet_property([Field = #{type := enum} | T], List) ->
    Validation = make_data_validation(Field),
    make_sheet_property(T, [Validation | List]);
%% other column
...
%% <Table></Table>
make_table(Prepend = #xmlElement{attributes = PrependAttributes}, Table) ->
    ...
    Attributes = listing:key_unique(#xmlAttribute.name, lists:merge(DefaultAttributes, PrependAttributes)),
...
%% <DataValidation></DataValidation>
make_data_validation(Field = #{column := Column}) ->
...
%% <Value></Value>
make_value(#{type := enum, validation := Validation}) ->
    Value = lists:concat([
        "\"", string:join([Value || {Value, _} <- Validation], ","), "\""
    ]),
...
to_collection(File, _) ->
    ...
    %% take book data
    Book = #{sheet := SheetData} = take_book(XmlData),

    %% import all sheet
    lists:foreach(fun({Name, Sheet}) -> import_table(Name, join(restore(Book, Name, Sheet))) end, maps:to_list(SheetData)).
...
to_table(File, Name) ->
    ...
    journal:print_row_table(Sheet),
    %% restore validation data
    RestoreSheet = restore(Book, Name, Sheet),
    journal:print_row_table(RestoreSheet),erlang:error(0),
    %% join column
...
restore_validation([#{type := list, column := Column} | T], Book = #{property := CustomProperty}, SheetName, Column, Cell) ->
    Key = lists:flatten(lists:concat([SheetName, "-", Column])),
    ValueKeyMap = maps:get(Key, CustomProperty),
    NewCell = string:join([maps:get(Option, ValueKeyMap, Option) || Option <- string:tokens(Cell, ",")], ","),
...
take_custom_property([], Validation) ->
    Validation;
take_custom_property([#xmlElement{name = 'CustomDocumentProperties', content = Content} | T], Validation) ->
    NewValidation = take_validation_property(Content, Validation),
    take_custom_property(T, NewValidation);
take_custom_property([_ | T], Property) ->
    take_custom_property(T, Property).


take_validation_property([], Validation) ->
    Validation;
take_validation_property([H = #xmlElement{name = Name} | T], Validation) ->
    case catch parser:evaluate(lists:flatten(take_text(H))) of
        {'EXIT', _} ->
            take_validation_property(T, Validation);
        ValueKeyMap ->
            %% [SheetName, Column] = string:tokens(type:to_list(Property), "-"),
            NewValidation = Validation#{Name => ValueKeyMap},
            take_validation_property(T, NewValidation)
    end;
take_validation_property([_ | T], Validation) ->
    take_validation_property(T, Validation).
