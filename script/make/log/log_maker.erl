...
parse_save([Table | T], CodeList) ->
    ...
    Code = io_lib:format("save(~s, Binding) ->\n    db:save_into(<<\"~s\">>, <<\"(~s)\">>, <<>>, Binding, fun(A) -> A end, 0);", [Table, SaveSql, InsertFormat]),
...
parse_replace([Table | T], Preset, CodeList) ->
    ...
    Code = io_lib:format("replace(~s, Data) ->\n    db:collect(<<\"REPLACE INTO (~s) VALUES \">>, <<\"~s\">>, <<>>, Data, fun(A) -> A end, 0);", [Table, InsertFields, InsertFormat]),
