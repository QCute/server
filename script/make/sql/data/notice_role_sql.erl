...
save(NoticeRoleList) ->
    db:save_into(<<"INSERT INTO `notice_role` (`role_id`, `notice_id`, `read_time`) VALUES">>, <<"(:1:, :2:, :5:)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `notice_id` = VALUES(`notice_id`), `read_time` = VALUES(`read_time`)">>, NoticeRoleList, #notice_role.flag).
...
update_read(NoticeRole) ->
    db:update(<<"UPDATE `notice_role` SET `read_time` = :5: WHERE `role_id` = :1: AND `notice_id` = :2:">>, NoticeRole).
