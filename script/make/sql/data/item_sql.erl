...
save(ItemList) ->
    db:save_into(<<"INSERT INTO `item` (`item_no`, `role_id`, `item_id`, `type`, `number`, `expire_time`) VALUES">>, <<"(:1:, :2:, :3:, :4:, :5:, :6:)">>, <<"ON DUPLICATE KEY UPDATE `item_no` = VALUES(`item_no`), `role_id` = VALUES(`role_id`), `item_id` = VALUES(`item_id`), `type` = VALUES(`type`), `number` = VALUES(`number`), `expire_time` = VALUES(`expire_time`)">>, ItemList, #item.flag).
...
delete_in_item_no(ItemNo) ->
    db:delete(<<"DELETE FROM `item` WHERE `item_no` IN (?)">>, [db:in(ItemNo)]).
