...
insert(Notice) ->
    db:insert(<<"INSERT INTO `notice` (`notice_id`, `type`, `receive_time`, `expire_time`, `title`, `content`, `attachment`, `from`) VALUES (:1:, :2:, :3:, :4:, :5:, :6:, :7:, :8:)">>, Notice).
...
delete_in_notice_id(NoticeId) ->
    db:delete(<<"DELETE FROM `notice` WHERE `notice_id` IN (?)">>, [db:in(NoticeId)]).
