...
save(FriendList) ->
    db:save_into(<<"INSERT INTO `friend` (`role_id`, `friend_role_id`, `relation`, `time`) VALUES">>, <<"(:1:, :2:, :10:, :11:)">>, <<"ON DUPLICATE KEY UPDATE `role_id` = VALUES(`role_id`), `friend_role_id` = VALUES(`friend_role_id`), `relation` = VALUES(`relation`), `time` = VALUES(`time`)">>, FriendList, #friend.flag).
...
update_relation(Friend) ->
    db:update(<<"UPDATE `friend` SET `relation` = :10: WHERE `role_id` = :1: AND `friend_role_id` = :2:">>, Friend).
