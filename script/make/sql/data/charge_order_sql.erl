...
insert(ChargeOrder) ->
    db:insert(<<"INSERT INTO `charge_order` (`charge_no`, `charge_id`, `order_id`, `channel`, `role_id`, `role_name`, `money`, `status`, `time`) VALUES (:1:, :2:, :3:, :4:, :5:, :6:, :7:, :8:, :9:)">>, ChargeOrder).
...
update_status(ChargeOrder) ->
    db:update(<<"UPDATE `charge_order` SET `status` = :8: WHERE `charge_no` = :1:">>, ChargeOrder).
