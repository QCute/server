...
parse_sql(File, SQL = #{delete := [], from := Table, as := FunctionName}, Order) ->
    ...
    Params = string:join([type:to_list(Transform) || #field{transform = Transform} <- Keys], ", "),
...
%% compare only
parse_where_compare(File, SQL, Table, Operation, Name, Compare = in, Fields) ->
...
parse_where_compare(File, SQL, Table, Operation, Name, Compare = between, Fields) ->
    #field{alias = Alias, value = Format} = parse_field_format(File, SQL, Table, Operation, Fields, Name),
...
%% not literal
parse_where_compare_literal(File, SQL, Table, Operation, Name, Compare = exists, {'$param$', []}, Fields) ->
    #field{alias = Alias, value = Format} = parse_field_format(File, SQL, Table, Operation, Fields, Name),
    lists:concat([type:to_list(Alias), " ", string:to_upper(type:to_list(Compare)), " (", type:to_list(Format), ")"]);
...
%% compare only
parse_having_compare(File, SQL, Table, Operation, Name, Compare = in, Fields) ->
    #field{alias = Alias, value = Format} = parse_field_format(File, SQL, Table, Operation, Fields, Name),
    lists:concat([type:to_list(Alias), " ", string:to_upper(type:to_list(Compare)), " (", type:to_list(Format), ")"]);

parse_having_compare(File, SQL, Table, Operation, Name, Compare = between, Fields) ->
    #field{alias = Alias, value = Format} = parse_field_format(File, SQL, Table, Operation, Fields, Name),
    lists:concat([type:to_list(Alias), " ", string:to_upper(type:to_list(Compare)), " ", type:to_list(Format), " AND ", type:to_list(Format)]);
...
%% not literal
parse_having_compare_literal(File, SQL, Table, Operation, Name, Compare = exists, {'$param$', []}, Fields) ->
    #field{alias = Alias, value = Format} = parse_field_format(File, SQL, Table, Operation, Fields, Name),
    lists:concat([type:to_list(Alias), " ", string:to_upper(type:to_list(Compare)), " (", type:to_list(Format), ")"]);
...
%% compare only
parse_key_compare(File, SQL, Table, Operation, Name, Compare = in, Fields) ->
    Field = #field{alias = Alias} = parse_field_type(File, SQL, Table, Operation, Fields, Name),
    Transform = lists:concat(["db:in(", type:to_list(Alias), ")"]),
    [Field#field{transform = Transform, preset = Compare}];

parse_key_compare(File, SQL, Table, Operation, Name, Compare = between, Fields) ->
    Field = #field{alias = Alias} = parse_field_type(File, SQL, Table, Operation, Fields, Name),
