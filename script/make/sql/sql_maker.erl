...
parse_sql(File, SQL = #{select := Select, from := Table, as := FunctionName}, MapMeta) ->
    ...
    %% arg use origin keys
    Arg = string:join([type:to_list(Transform) || #field{transform = Transform} <- KeyFields], ", "),
...
parse_sql(File, SQL = #{change := Change, into := Table, as := FunctionName}, MapMeta) ->
    ...
    KeyFields = parse_key(File, SQL, Table, change, Fields, maps:get(by, MapMeta, []), maps:get(having, MapMeta, [])),
...
parse_field_value(File, SQL, Table, Operation, Fields, Name, Preset) ->
    ...
    Value = maps:get(Operation, #{select => Alias, change => "?"}, lists:concat([":", Field#field.position, ":"])),
...
parse_field_format(Field = #field{position = Position}, insert) ->
    Field#field{value = lists:concat([":", Position, ":"])};
