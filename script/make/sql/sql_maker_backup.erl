...
parse_code_update(TableName, Keys, _Fields) ->
    ...
    Sql = <<(db:format(element(1, ?UPDATE_~s), ~s))/binary, (db:format(element(2, ?UPDATE_~s), [~s]))/binary>>,
...
parse_code_select_join(TableName, ArgKeys, [], _SelectJoinKeys) ->
    ...
    Sql = db:format(?SELECT_JOIN_~s, [~s]),
    Data = db:select(Sql),
