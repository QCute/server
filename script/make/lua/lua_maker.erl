...
parse_file(#{file := File, sql := Sql}) ->
    Order = maker:collect_map_order("script/make/lua/lua_script.erl", File),
...
parse_sql(File, SQL = #{select := Select, from := Table, as := FunctionName}, Order) ->
    ...
    parse_key_value(File, SQL, Select, Table, Fields, FunctionName, KeyFields, Range, ValueFields, Result).


parse_key_value(File, SQL, Select, Table, Fields, FunctionName, [], _, ValueFields, ValueSql) ->
...
format_key(_, _, _, _, _, _, _, Depth, Leaf, 1, 1, [], Value, _, Guard) ->
    ...
    Condition = string:join(lists:reverse(Guard), " and "),
    lists:concat([
        Padding, "if ", Condition, " then", "\n",
        Align, Value, "\n",
        Padding, "else", "\n",
        Padding, "    ", "return nil", "\n",
        Padding, "end"
...
%% convert erlang term to lua table
convert(<<>>, <<>>, _, Acc) ->
    Acc;
