...
send_query_self_apply(User, List) ->
    {ok, Binary} = guild_protocol:encode(30106, List),
    User#user{buffer = <<(User#user.buffer)/binary, Binary/binary>>}.

send_create(User, Result) ->
