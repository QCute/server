...
send_heartbeat(Client, Result) ->
    {ok, Binary} = account_protocol:encode(10000, Result),
...
send_login(Client, Result) ->
    {ok, Binary} = account_protocol:encode(10003, Result),
    sender:send(Client, Binary).

send_logout(Client, Result) ->
    {ok, Binary} = account_protocol:encode(10004, Result),
    sender:send(Client, Binary).
