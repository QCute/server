...
decode_target_list_20014(_Rest_ = <<_/binary>>, Size, TargetListLength, List) ->
    <<TargetListItem:64, _TargetListItemRest_/binary>> = _Rest_,
    decode_target_list_20014(_TargetListItemRest_, Size + byte_size(_Rest_) - byte_size(_TargetListItemRest_), TargetListLength - 1, [TargetListItem | List]).
...
encode_data_20011(Acc = <<_/binary>>, Length, [#fighter{id = Id, type = Type, attribute = #attribute{fc = Fc, hp = Hp, health = Health}, skill = Skill, buff = Buff, x = X, y = Y} | Data]) ->
    encode_data_20011(<<Acc/binary, Id:64, Type:8, Fc:64, Hp:64, Health:64, (encode_skill_20011(<<>>, 0, Skill))/binary, (encode_buff_20011(<<>>, 0, Buff))/binary, X:16, Y:16>>, Length + 1, Data).
...
encode_fighter_list_20014(Acc = <<_/binary>>, Length, []) ->
    <<Length:16, Acc/binary>>;
encode_fighter_list_20014(Acc = <<_/binary>>, Length, [#fighter{id = Id, type = Type, attribute = #attribute{fc = Fc, hp = Hp, health = Health}, skill = Skill, buff = Buff, x = X, y = Y} | FighterList]) ->
    encode_fighter_list_20014(<<Acc/binary, Id:64, Type:8, Fc:64, Hp:64, Health:64, (encode_skill_20014(<<>>, 0, Skill))/binary, (encode_buff_20014(<<>>, 0, Buff))/binary, X:16, Y:16>>, Length + 1, FighterList).
