...
encode(11602, ) ->
    Data11602 = <<(encode__11602(<<>>, 0, ))/binary>>,
...
encode(11604, ) ->
    Data11604 = <<(encode__11604(<<>>, 0, ))/binary>>,
...
encode(11606, ) ->
    Data11606 = <<(encode__11606(<<>>, 0, ))/binary>>,
...
encode(11608, ) ->
    Data11608 = <<(encode__11608(<<>>, 0, ))/binary>>,
    {ok, <<(byte_size(Data11608)):16, 11608:16, Data11608/binary>>};

encode(Protocol, Data) ->
    {error, Protocol, Data}.

encode__11602(Acc = <<_/binary>>, Length, []) ->
    <<Length:16, Acc/binary>>;
encode__11602(Acc = <<_/binary>>, Length, [#system_chat{id = Id, role_id = RoleId, role_name = RoleName, type = Type, message = Message} | ]) ->
    encode__11602(<<Acc/binary, Id:64, RoleId:64, (byte_size(RoleName)):16, (RoleName)/binary, Type:8, (byte_size(Message)):16, (Message)/binary>>, Length + 1, ).

encode__11604(Acc = <<_/binary>>, Length, []) ->
    <<Length:16, Acc/binary>>;
encode__11604(Acc = <<_/binary>>, Length, [#world_chat{id = Id, role_id = RoleId, role_name = RoleName, type = Type, message = Message} | ]) ->
    encode__11604(<<Acc/binary, Id:64, RoleId:64, (byte_size(RoleName)):16, (RoleName)/binary, Type:8, (byte_size(Message)):16, (Message)/binary>>, Length + 1, ).

encode__11606(Acc = <<_/binary>>, Length, []) ->
    <<Length:16, Acc/binary>>;
encode__11606(Acc = <<_/binary>>, Length, [#guild_chat{id = Id, role_id = RoleId, role_name = RoleName, type = Type, message = Message} | ]) ->
    encode__11606(<<Acc/binary, Id:64, RoleId:64, (byte_size(RoleName)):16, (RoleName)/binary, Type:8, (byte_size(Message)):16, (Message)/binary>>, Length + 1, ).

encode__11608(Acc = <<_/binary>>, Length, []) ->
    <<Length:16, Acc/binary>>;
encode__11608(Acc = <<_/binary>>, Length, [#private_chat{sender_id = SenderId, receiver_id = ReceiverId, type = Type, message = Message} | ]) ->
    encode__11608(<<Acc/binary, SenderId:64, ReceiverId:64, Type:8, (byte_size(Message)):16, (Message)/binary>>, Length + 1, ).
