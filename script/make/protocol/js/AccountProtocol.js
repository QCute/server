        ...
        switch (protocol) {
            ...
            case 10001: {
                ...
                while (--listLength >= 0) {
                    // 
                    // 角色ID
                    const listRoleId = view.getBigUint64(offset, false);
                    offset = offset + 8;
                    // 角色名
                    const listRoleNameLength = view.getUint16(offset, false);
                    offset = offset + 2;
                    const listRoleNameArray = new Uint8Array(view.buffer.slice(offset, offset + listRoleNameLength));
                    const listRoleName = textDecoder.decode(listRoleNameArray);
                    offset = offset + listRoleNameLength;
                    // object
                    const listItem = {"roleId": listRoleId, "roleName": listRoleName};
                    // add
                    list.push(listItem);
                }
                // object
                const data = {"result": result, "list": list};
                return data;
            }
            case 10002: {
                // 
                ...
                // object
                const data = {"result": result, "roleId": roleId, "roleName": roleName};
                return data;
            }
            case 10003: {
                // 结果
