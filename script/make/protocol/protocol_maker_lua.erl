...
parse_encode_lua_loop([#meta{name = Name, type = rst, explain = undefined, comment = Comment} | T], Depth, Parent, Ancestor, Scope, Fields, List) ->
    %% alignment padding
    Padding = lists:duplicate(Depth, "    "),
    NewAncestor = [Name | Ancestor],
    PathNames = lists:reverse(lists:sublist(NewAncestor, max(1, length(NewAncestor) - 1))),
...
parse_decode_lua_loop([#meta{name = Name, type = rst, explain = undefined, comment = Comment} | T], Depth, Parent, Ancestor, Fields, List) ->
    %% alignment padding
    Padding = lists:duplicate(Depth, "    "),
    NewAncestor = [Name | Ancestor],
    PathNames = lists:reverse(lists:sublist(NewAncestor, max(1, length(NewAncestor) - 1))),
