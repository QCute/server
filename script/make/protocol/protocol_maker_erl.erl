...
parse_decode_erl_loop([#meta{name = Name, type = list, explain = Explain, key = _} | T], Protocol, Depth, Parent, Ancestor, Scope, Records, Functions, Names, List) ->
    ...
    SubFunction = lists:concat([
        ...
        "    ", "{Size, List};", "\n",
        "decode_", PathSnakeName, "_", Protocol, "(_Rest_ = <<_/binary>>, Size, ", PathHumpName, "Length, List) ->", "\n",
        string:join(SubCodes, "\n"), "\n",
        "    ", "decode_", PathSnakeName, "_", Protocol, "(_", NextScope, "Rest_, Size + byte_size(_Rest_) - byte_size(_", NextScope, "Rest_), ", PathHumpName, "Length - 1, ", "[", string:join(SubNames, ", "), " | List]).", "\n",
