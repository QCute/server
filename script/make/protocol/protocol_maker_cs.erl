...
parse_encode_cs_loop([#meta{name = Name, type = record, explain = Explain} | T], Depth, Parent, Ancestor, Scope, Names, List) ->
    Padding = lists:duplicate(Depth, "    "),
    NewAncestor = [Name | Ancestor],
    PathNames = lists:reverse(lists:sublist(NewAncestor, max(1, length(NewAncestor) - 1))),
    PathHumpName = word:to_lower_hump(string:join([word:to_hump(PathName) || PathName <- PathNames], "")),
    ...
    %% flatten
    parse_encode_cs_loop(T, Depth, Parent, Ancestor, Scope, lists:reverse(SubNames), lists:reverse([SubCode | SubCodes]));
...
parse_decode_cs_loop([#meta{name = Name, type = record, explain = Explain, comment = Comment} | T], Depth, Parent, Ancestor, Fields, List) ->
    %% alignment padding
    Padding = lists:duplicate(Depth, "    "),
    NewAncestor = [Name | Ancestor],
    PathNames = lists:reverse(lists:sublist(NewAncestor, max(1, length(NewAncestor) - 1))),
