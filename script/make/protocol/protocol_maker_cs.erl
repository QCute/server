...
parse_class_loop([Meta = #meta{type = tuple, explain = Explain} | T], Depth, Names, List) ->
    ...
    SubCode = fun
        Loop([#meta{name = SubName, comment = SubComment} | X], [SubCode | Y], Acc, Position, Length) ->
            %% trim last comma
            Comma = ["," || Position =/= Length],
            %% the whole data
            ...
            Loop(X, Y, [Data | Acc], Position + 1, Length);
        Loop([], [], Acc, _, _) ->
            Acc
    end(SubNames, SubCodes, [], 1, length(SubNames)),
...
parse_encode_loop([#meta{from = From, path = Path, prefix = Prefix, suffix = Suffix, get = Get, type = tuple, explain = Explain} | T], Depth, Names, Type, List) ->
    ...
    TypeName = lists:concat(["(", string:join([[TypeName, " ", word:to_lower_hump(FieldName)] || {#meta{name = FieldName}, TypeName} <- lists:zip(SubExplain, SubTypes)], ", "), ")"]),
...
parse_decode(Protocol, Meta, #handler{}, _) ->
    ...
    %% codes format
    CodesDefault = lists:concat([
        Padding, "    ", "return ", "(", "data: ", string:join(Fields, ", "), ")", ";", "\n"
    ]),
    CodesBlock = lists:append(Codes, [CodesDefault]),
...
parse_decode_loop([#meta{path = Path, type = tuple, explain = Explain, comment = Comment} | T], Depth, Fields, Type, List) ->
    ...
    Code = [
        ...
        string:join(SubCodes, "\n"), "\n",
        Padding, "// object", "\n",
        Padding, "var ", PathHumpName, " = ", "(" ,string:join([lists:concat([word:to_lower_hump(FieldName), ": ", PathName]) || {#meta{name = FieldName}, PathName} <- lists:zip(SubExplain, SubFields)], ", "), ");"
    ],
