...
%%% API functions
%%%===================================================================
start(Path, OutFile, HeaderFile, HtmlFile, LuaFile, LuaMetaFile, JsFile, JsMetaFile, CsFile, CsMetaFile, IgnoreList) ->
    ...
    write_lua_meta_code(LuaMetaFile, Result),
    %% js protocol define
    write_js_code(JsFile, Result),
    write_js_meta_code(JsMetaFile, Result),
    %% cs protocol define
    write_cs_code(CsFile, Result),
    write_cs_meta_code(CsMetaFile, Result),
    %% html protocol define
    write_html_code(HtmlFile, Result),
    %% protocol define
    write_header_code(HeaderFile, Result),
...
replace_code(OutFile, Result, ReadCode, WriteCode, RouteCode) ->
    ...
    %% interval record
    IntervalRecord = io_lib:format("-record(protocol_interval, {~s}).", [string:join([lists:concat(["'", Protocol, "' = 0"]) || {Protocol, [_], _} <- lists:append([List || {_, _, List, _} <- Result])], ", ")]),
...
write_header_code(HeaderFile, List) ->
    Code = string:join([Code || Code <- [write_header_code_loop(lists:reverse(NameList), Name, []) || {_, _, NameList, Name} <- List], Code =/= []], "\n") ++ "\n\n",
...
<body>
    ...
    file:write_file(maker:relative_path(HtmlFile), unicode:characters_to_binary(lists:flatten(Code))).

%%%====================================================================
%%% Lua Define Part
%%%====================================================================
%% write lua protocol function
write_lua_code(LuaFile, List) ->
    ...
    file:write_file(maker:relative_path(LuaFile), lists:flatten([RequireCode, "\n\n", EncodeFunction, "\n\n", DecodeFunction])).

%% write lua protocol define function
write_lua_meta_code(LuaFile, List) ->
    RequireCode = string:join([io_lib:format("require(\"./~sProtocol\")", [word:to_hump(Name)]) || {_, _, _, Name} <- List], "\n"),
...
write_js_code(JsFile, List) ->
    ...
    file:write_file(maker:relative_path(JsFile), lists:flatten([ImportCode, "\n\n", EncodeFunction, "\n\n", DecodeFunction])).

%% write js protocol define function
write_js_meta_code(JsFile, List) ->
    ...
    Cast = "public static class Cast
{
    ...
    file:write_file(maker:relative_path(CsFile), lists:concat([Code, "\n", "\n", Cast])).

%% write cs protocol define function
write_cs_meta_code(CsFile, List) ->
