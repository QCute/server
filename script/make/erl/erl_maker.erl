...
parse_key_value(File, SQL, Select, Table, Fields, FunctionName, KeyFields, ValueFields, ValueSql, _, FilterHaving, UniqueBy, SortBy) ->

    %% collect keys
    KeyData = [db:select(lists:concat(["SELECT", " ", "DISTINCT", " ", "`", db:to_snake(Alias), "`", " ", "FROM", " ", "`", Table, "`", UniqueBy, FilterHaving, SortBy])) || #field{alias = Alias} <- KeyFields],
