...
parse_sql(File, SQL = #{select := Select, from := Table, as := FunctionName}, Order) ->
    ...
    parse_key_value(File, SQL, Select, Table, Fields, FunctionName, KeyFields, ValueFields, Result).


parse_key_value(File, SQL, Select, Table, Fields, FunctionName, [], ValueFields, ValueSql) ->
...
parse_key_value(File, SQL, Select, Table, Fields, FunctionName, KeyFields, ValueFields, ValueSql) ->
    ...
    List = collect_data(Matrix, ValueSql, KeyFields, ValueFields, []),

    Range = lists:any(fun(#field{preset = Preset}) -> listing:is_in(Preset, ['>', '<', '>=', '<=', '=<']) end, KeyFields),

    Return = string:join(format_row(File, SQL, Select, Table, Fields, FunctionName, Range, 1, length(List), 1, List, []), ";\n"),
...
format_spec(_, _, {'$all$', _}, _, _, _, ValueFields) ->
...
format_default(_, #{default := {'$raw$', Query}}, _, _, KeyFields, FunctionName) ->
