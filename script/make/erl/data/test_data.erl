...
-spec text(Key :: term()) -> {term(), binary(), binary()}.
...
-spec text_list() -> [{term(), binary(), binary()}].
text_list() ->
    [
        {account_create_max, <<"服务器角色数量已达到上限"/utf8>>, <<"文本"/utf8>>},
        {account_login_forbidden, <<"账号禁止登录"/utf8>>, <<"文本"/utf8>>},
...
-spec type_list() -> [non_neg_integer()].
type_list() ->
    [1, 2, 3, 4, 5, 6].

-spec min_exp_max_level() -> {non_neg_integer(), non_neg_integer()}.
min_exp_max_level() ->
    {100, 9}.

-spec text_count() -> non_neg_integer().
text_count() ->
    102.

-spec max_text() -> {term(), binary()}.
max_text() ->
    {vip_level_not_met, <<"附件为空"/utf8>>}.

-spec ref(Key :: term(), Value :: binary()) -> binary().
...
-spec ref_range(Key :: term(), Value :: binary()) -> [binary()].
ref_range(condition, Value) when Value < <<"{level, n}"/utf8>> ->
    [<<"职业为n"/utf8>>];
ref_range(condition, Value) when Value < <<"{sex, n}"/utf8>> ->
    [<<"职业为n"/utf8>>, <<"等级n级"/utf8>>];
ref_range(condition, Value) when Value < <<"{vip, n}"/utf8>> ->
    [<<"职业为n"/utf8>>, <<"等级n级"/utf8>>, <<"性别为n"/utf8>>];
ref_range(_, _) ->
    undefined.

-spec get_level_by_exp_asc(Exp :: non_neg_integer()) -> non_neg_integer().
